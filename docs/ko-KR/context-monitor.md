...
| CRITICAL | <= 25% | 즉시 중단 후 상태 저장 (`/thrunt:pause-work`) |
...
THRUNT의 `/thrunt:pause-work` 명령어는 실행 상태를 저장합니다. WARNING 메시지는 해당 명령어 사용을 권장하며 CRITICAL 메시지는 즉각적인 상태 저장을 지시합니다.

## 설정

두 훅 모두 `npx thrunt-god` 설치 중에 자동으로 등록됩니다.
