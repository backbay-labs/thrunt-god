...
hunt-plan 조사 단계에서 THRUNT는 코드 작성 전에 각 페이즈 요구사항에 대한 자동화된 테스트 커버리지를 매핑합니다. 이를 통해 Claude의 실행자가 작업을 커밋할 때 몇 초 안에 검증할 수 있는 피드백 메커니즘이 이미 갖춰져 있습니다.
...
**비활성화:** 테스트 인프라가 중요하지 않은 빠른 프로토타이핑 페이즈에서는 `/thrunt:settings`에서 `workflow.nyquist_validation: false`로 설정하세요.

### 소급 검증 (`/thrunt:validate-phase`)
...
**사용 시점:** Nyquist가 활성화되기 전에 계획된 페이즈를 실행한 후 또는 `/thrunt:audit-milestone`에서 Nyquist 준수 갭이 발견된 후에 사용합니다.

### 가정 토론 모드

기본적으로 `/hunt:shape-hypothesis`는 구현 선호도에 대한 개방형 질문을 합니다. 가정 모드는 이를 역전시킵니다. THRUNT가 먼저 코드베이스를 읽고 페이즈를 어떻게 구축할지에 대한 구조화된 가정을 제시한 후 수정사항만 요청합니다.

**활성화:** `/thrunt:settings`에서 `workflow.discuss_mode`를 `'assumptions'`로 설정합니다.

**작동 방식.**
1. MISSION.md, 코드베이스 매핑, 기존 관례를 읽습니다.
...
`/thrunt:ui-phase`는 계획 전에 설계 계약을 확정합니다. `/thrunt:ui-review`는 실행 후 결과를 감사합니다.

### 명령어

| 명령어 | 설명 |
|--------|------|
| `/thrunt:ui-phase [N]` | 프론트엔드 페이즈를 위한 UI-SPEC.md 설계 계약 생성 |
| `/thrunt:ui-review [N]` | 구현된 UI의 6개 기둥 기반 시각적 감사 소급 수행 |

### 워크플로우: `/thrunt:ui-phase`

**실행 시점:** `/hunt:shape-hypothesis` 이후, `/hunt:plan` 이전 — 프론트엔드/UI 작업이 포함된 페이즈.

**흐름.**
1. CONTEXT.md, RESEARCH.md, HYPOTHESES.md에서 기존 결정사항을 읽습니다.
...
### 워크플로우: `/thrunt:ui-review`

**실행 시점:** `/hunt:run` 또는 `/hunt:validate-findings` 이후 — 프론트엔드 코드가 있는 모든 프로젝트.

**독립 실행:** 모든 프로젝트에서 작동하며 THRUNT 관리 프로젝트가 아니어도 됩니다. UI-SPEC.md가 없으면 추상적인 6개 기둥 기준으로 감사합니다.
...
| `workflow.ui_safety_gate` | `true` | hunt-plan가 프론트엔드 페이즈에서 /thrunt:ui-phase 실행을 유도합니다 |

두 설정 모두 부재 시 활성화 패턴을 따릅니다. `/thrunt:settings`에서 비활성화할 수 있습니다.
...
프리셋 문자열은 THRUNT의 1급 계획 아티팩트가 되어 페이즈와 마일스톤에 걸쳐 재현 가능합니다.
...
`/thrunt:ui-review`는 Playwright CLI를 통해 `.planning/ui-reviews/`에 스크린샷을 캡처합니다. 바이너리 파일이 git에 포함되지 않도록 `.gitignore`가 자동으로 생성됩니다. 스크린샷은 `/thrunt:complete-milestone` 실행 시 정리됩니다.
...
백로그 항목은 전체 페이즈 디렉터리를 얻으므로 `/hunt:shape-hypothesis 999.1`로 아이디어를 더 탐구하거나 준비가 되면 `/hunt:plan 999.1`을 사용할 수 있습니다.

`/thrunt:review-backlog`으로 **검토 및 승격**합니다 — 모든 백로그 항목을 표시하고 승격 (활성 순서로 이동), 유지 (백로그에 남김), 또는 제거 (삭제)를 선택할 수 있습니다.
...
시드는 전체 WHY와 언제 표면화할지를 보존합니다. `/hunt:new-program`은 모든 시드를 스캔하여 일치 항목을 제시합니다.
...
스레드는 `/thrunt:pause-work`보다 가볍습니다. 페이즈 상태나 계획 컨텍스트가 없습니다. 각 스레드 파일에는 목표, 컨텍스트, 참조, 다음 단계 섹션이 포함됩니다.

스레드가 성숙해지면 페이즈(`/thrunt:add-phase`)나 백로그 항목(`/thrunt:add-backlog`)으로 승격할 수 있습니다.
...
| 명령어 | 목적 |
|--------|------|
| `/thrunt:workstreams create <name>` | 격리된 계획 상태로 새 워크스트림 생성 |
| `/thrunt:workstreams switch <name>` | 활성 컨텍스트를 다른 워크스트림으로 전환 |
| `/thrunt:workstreams list` | 모든 워크스트림과 활성 워크스트림 표시 |
| `/thrunt:workstreams complete <name>` | 워크스트림을 완료로 표시하고 상태 아카이브 |

### 작동 방식

각 워크스트림은 자체 `.planning/` 디렉터리 하위 트리를 유지합니다. 워크스트림을 전환하면 THRUNT가 활성 계획 컨텍스트를 교체하여 `/thrunt:progress`, `/hunt:shape-hypothesis`, `/hunt:plan` 및 기타 명령어가 해당 워크스트림의 상태로 동작합니다.

이는 `/thrunt:new-workspace`(별도 저장소 worktree를 생성)보다 가볍습니다. 워크스트림은 동일한 코드베이스와 git 히스토리를 공유하지만 계획 아티팩트를 격리합니다.
...
THRUNT는 LLM 시스템 프롬프트가 되는 마크다운 파일을 생성합니다. 즉 계획 아티팩트로 유입되는 사용자 제어 텍스트는 잠재적인 간접 프롬프트 인젝션 벡터입니다. v1.27에서 중앙화된 보안 강화가 도입되었습니다.
...
**런타임 훅.**
- `thrunt-prompt-guard.js` — `.planning/`에 대한 Write/Edit 호출에서 인젝션 패턴 스캔 (항상 활성, 권고만)
- `thrunt-workflow-guard.js` — THRUNT 워크플로우 컨텍스트 밖의 파일 편집 시 경고 (`hooks.workflow_guard`로 선택적 활성화)
...
| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/hunt:new-program` | 전체 프로젝트 초기화: 질문, 조사, 요구사항, 로드맵 | 새 프로젝트 시작 시 |
| `/hunt:new-program --auto @idea.md` | 문서에서 자동 초기화 | PRD나 아이디어 문서가 준비된 경우 |
| `/hunt:shape-hypothesis [N]` | 구현 결정사항 캡처 | 계획 전 구축 방식을 결정할 때 |
| `/thrunt:ui-phase [N]` | UI 설계 계약 생성 | shape-hypothesis 이후, hunt-plan 이전 (프론트엔드 페이즈) |
| `/hunt:plan [N]` | 조사 + 계획 + 검증 | 페이즈 실행 전 |
| `/hunt:run <N>` | 병렬 웨이브로 모든 계획 실행 | 계획이 완료된 후 |
| `/hunt:validate-findings [N]` | 자동 진단을 포함한 수동 Evidence Review | 실행 완료 후 |
| `/hunt:publish [N]` | 검증된 작업으로 PR 생성 | 검증 통과 후 |
| `/thrunt:fast <text>` | 계획을 완전히 건너뛰는 인라인 간단 작업 | 오타 수정, 설정 변경, 소규모 리팩터링 |
| `/thrunt:next` | 상태 자동 감지 및 다음 단계 실행 | 언제든 — "다음에 무엇을 해야 하나?" |
| `/thrunt:ui-review [N]` | 6개 기둥 기반 시각적 감사 소급 수행 | 실행 또는 validate-findings 이후 (프론트엔드 프로젝트) |
| `/thrunt:audit-milestone` | 마일스톤이 완료 정의를 충족했는지 검증 | 마일스톤 완료 전 |
| `/thrunt:complete-milestone` | 마일스톤 아카이브 및 릴리스 태그 생성 | 모든 페이즈 검증 완료 시 |
| `/hunt:new-program [name]` | 다음 버전 사이클 시작 | 마일스톤 완료 후 |

### 탐색

| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/thrunt:progress` | 상태 및 다음 단계 표시 | 언제든 -- "지금 어디 있나?" |
| `/thrunt:resume-work` | 마지막 세션의 전체 컨텍스트 복원 | 새 세션 시작 시 |
| `/thrunt:pause-work` | 구조화된 핸드오프 저장 (HANDOFF.json + continue-here.md) | 페이즈 중간에 중단할 때 |
| `/thrunt:session-report` | 작업 및 결과가 포함된 세션 요약 생성 | 세션 종료 시, 이해관계자 공유 시 |
| `/thrunt:help` | 모든 명령어 표시 | 빠른 레퍼런스 |
| `/thrunt:update` | 변경 로그 미리보기와 함께 THRUNT 업데이트 | 새 버전 확인 시 |
| `/thrunt:join-discord` | Discord 커뮤니티 초대 링크 열기 | 질문이나 커뮤니티 참여 시 |

### 페이즈 관리

| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/thrunt:add-phase` | 로드맵에 새 페이즈 추가 | 초기 계획 후 범위가 늘어날 때 |
| `/thrunt:insert-phase [N]` | 긴급 작업 삽입 (소수점 번호 체계) | 마일스톤 중간의 긴급 수정 시 |
| `/thrunt:remove-phase [N]` | 미래 페이즈 제거 및 재번호 | 기능 범위 축소 시 |
| `/thrunt:list-phase-assumptions [N]` | Claude의 예상 접근 방식 미리 확인 | 계획 전 방향 검증 시 |
| `/thrunt:plan-milestone-gaps` | 감사 갭을 위한 페이즈 생성 | 감사에서 누락 항목이 발견된 후 |
| `/hunt:shape-hypothesis [N]` | 심층 에코시스템 조사만 수행 | 복잡하거나 익숙하지 않은 도메인 |

### 브라운필드 및 유틸리티

| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/hunt:map-environment` | 기존 코드베이스 분석 | 기존 코드에서 `/hunt:new-program` 실행 전 |
| `/thrunt:quick` | THRUNT 보증을 갖춘 임시 작업 | 버그 수정, 소규모 기능, 설정 변경 |
| `/thrunt:debug [desc]` | 지속적인 상태를 유지하는 체계적인 디버깅 | 문제가 발생했을 때 |
| `/thrunt:forensics` | 워크플로우 실패에 대한 진단 보고서 | 상태, 아티팩트, git 히스토리가 손상된 것 같을 때 |
| `/thrunt:add-todo [desc]` | 나중을 위한 아이디어 캡처 | 세션 중에 생각이 날 때 |
| `/thrunt:check-todos` | 보류 중인 할 일 목록 | 캡처된 아이디어 검토 시 |
| `/thrunt:settings` | 워크플로우 토글 및 모델 프로필 설정 | 모델 변경, 에이전트 토글 시 |
| `/thrunt:set-profile <profile>` | 빠른 프로필 전환 | 비용/품질 트레이드오프 변경 시 |
| `/thrunt:reapply-patches` | 업데이트 후 로컬 수정사항 복원 | 로컬 편집이 있는 상태에서 `/thrunt:update` 이후 |

### 코드 품질 및 리뷰

| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/thrunt:review --phase N` | 외부 CLI를 통한 교차 AI 동료 리뷰 | 실행 전 계획 검증 시 |
| `/thrunt:pr-branch` | `.planning/` 커밋을 필터링한 깔끔한 PR 브랜치 | 계획 없는 diff로 PR 생성 전 |
| `/thrunt:audit-evidence` | 모든 페이즈의 검증 부채 감사 | 마일스톤 완료 전 |

### 백로그 및 스레드

| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/thrunt:add-backlog <desc>` | 백로그 파킹 롯에 아이디어 추가 (999.x) | 활성 계획에 준비되지 않은 아이디어 |
| `/thrunt:review-backlog` | 백로그 항목 승격/유지/제거 | 새 마일스톤 전 우선순위 결정 시 |
| `/thrunt:plant-seed <idea>` | 트리거 조건이 있는 미래 지향적인 아이디어 | 미래 마일스톤에서 표면화되어야 할 아이디어 |
| `/thrunt:thread [name]` | 지속적인 컨텍스트 스레드 | 페이즈 구조 밖의 교차 세션 작업 |

---

## 설정 레퍼런스

THRUNT는 프로젝트 설정을 `.planning/config.json`에 저장합니다. `/hunt:new-program` 중에 설정하거나 나중에 `/thrunt:settings`로 업데이트할 수 있습니다.
...
| `workflow.validator` | `true`, `false` | `true` | 페이즈 목표에 대한 실행 후 검증 |
| `workflow.nyquist_validation` | `true`, `false` | `true` | hunt-plan 중 검증 아키텍처 조사 및 8번째 plan-check 차원 |
| `workflow.ui_phase` | `true`, `false` | `true` | 프론트엔드 페이즈를 위한 UI 설계 계약 생성 |
| `workflow.ui_safety_gate` | `true`, `false` | `true` | hunt-plan가 프론트엔드 페이즈에서 /thrunt:ui-phase 실행을 유도합니다 |
| `workflow.research_before_questions` | `true`, `false` | `false` | 토론 질문 이후가 아닌 이전에 조사를 실행합니다 |
| `workflow.discuss_mode` | `discuss`, `assumptions` | `discuss` | 토론 방식: 질문을 하나씩 진행 vs. 코드베이스 기반 가정 |
| `workflow.skip_discuss` | `true`, `false` | `false` | 자율 모드에서 shape-hypothesis를 완전히 건너뜁니다. HUNTMAP 페이즈 목표에서 최소한의 CONTEXT.md를 작성합니다 |
...
| `hooks.workflow_guard` | `true`, `false` | `false` | THRUNT 워크플로우 컨텍스트 밖의 파일 편집 시 경고 |
...
| `git.phase_branch_template` | 템플릿 문자열 | `thrunt/phase-{phase}-{slug}` | phase 전략의 브랜치 이름 |
| `git.milestone_branch_template` | 템플릿 문자열 | `thrunt/{milestone}-{slug}` | milestone 전략의 브랜치 이름 |
| `git.quick_branch_template` | 템플릿 문자열 또는 `null` | `null` | `/thrunt:quick` 작업의 선택적 브랜치 이름 |
...
| `phase` | 각 `hunt-run` 시 | 페이즈당 하나의 브랜치 | 페이즈별 코드 리뷰, 세분화된 롤백 |
| `milestone` | 첫 `hunt-run` 시 | 모든 페이즈가 하나의 브랜치 공유 | 릴리스 브랜치, 버전별 PR |
...
**자율 모드에서 shape-hypothesis 건너뛰기:** MISSION.md에 선호도가 이미 충분히 캡처된 `yolo` 모드에서 실행할 때 `/thrunt:settings`에서 `workflow.skip_discuss: true`로 설정하세요. 이렇게 하면 shape-hypothesis를 완전히 우회하고 HUNTMAP 페이즈 목표에서 파생된 최소한의 CONTEXT.md를 작성합니다. MISSION.md와 관례가 충분히 포괄적이어서 토론이 새로운 정보를 제공하지 않을 때 유용합니다.
...
격리된 THRUNT 상태로 여러 저장소나 기능을 병렬로 작업합니다.
...
`.planning/MISSION.md`가 이미 존재하는데 `/hunt:new-program`를 실행했습니다. 이것은 안전 검사입니다. 처음부터 다시 시작하려면 먼저 `.planning/` 디렉터리를 삭제하세요.

### 긴 세션 중 컨텍스트 저하

주요 명령어 사이에 컨텍스트 윈도우를 지우세요: Claude Code에서 `/clear`를 사용합니다. THRUNT는 새로운 컨텍스트를 기반으로 설계되었습니다 — 모든 서브에이전트는 깨끗한 200K 윈도우를 받습니다. 메인 세션의 품질이 저하되면 지우고 `/thrunt:resume-work` 또는 `/thrunt:progress`를 사용하여 상태를 복원하세요.

### 계획이 잘못되거나 맞지 않는 경우

계획 전에 `/hunt:shape-hypothesis [N]`을 실행하세요. 대부분의 계획 품질 문제는 `CONTEXT.md`가 있었다면 방지할 수 있었던 가정을 Claude가 세우기 때문에 발생합니다. `/thrunt:list-phase-assumptions [N]`을 실행하여 계획에 동의하기 전에 Claude가 무엇을 하려는지 확인할 수도 있습니다.
...
`/thrunt:progress`를 실행하세요. 모든 상태 파일을 읽고 현재 위치와 다음에 할 일을 정확히 알려줍니다.

### 실행 후 변경이 필요한 경우

`/hunt:run`를 다시 실행하지 마세요. 목표를 정확히 수정하려면 `/thrunt:quick`을 사용하거나 Evidence Review를 통해 체계적으로 문제를 식별하고 수정하려면 `/hunt:validate-findings`를 사용하세요.

### 모델 비용이 너무 높은 경우

예산 프로필로 전환하세요: `/thrunt:set-profile budget`. 도메인이 익숙하다면 (또는 Claude에게 익숙하다면) `/thrunt:settings`에서 조사 및 plan-check 에이전트를 비활성화하세요.

### 비Claude 런타임 사용 (Codex, OpenCode, Gemini CLI)

비Claude 런타임용으로 THRUNT를 설치했다면 설치 프로그램이 이미 모든 에이전트가 런타임의 기본 모델을 사용하도록 모델 해석을 구성했습니다. 수동 설정이 필요하지 않습니다. 구체적으로 설치 프로그램은 config에 `resolve_model_ids: "omit"`을 설정하여 THRUNT가 Anthropic 모델 ID 해석을 건너뛰고 런타임이 자체 기본 모델을 선택하도록 합니다.
...
THRUNT 서브에이전트가 Anthropic 모델을 호출하는데 OpenRouter나 로컬 공급자를 통해 비용을 지불하고 있다면 `inherit` 프로필로 전환하세요: `/thrunt:set-profile inherit`. 이렇게 하면 모든 에이전트가 특정 Anthropic 모델 대신 현재 세션 모델을 사용합니다. `/thrunt:settings` → Model Profile → Inherit도 참고하세요.

### 민감하거나 비공개 프로젝트에서 작업하는 경우

`/hunt:new-program` 중에 또는 `/thrunt:settings`에서 `commit_docs: false`로 설정하세요. `.planning/`을 `.gitignore`에 추가하세요. 계획 아티팩트는 로컬에 유지되며 git에 절대 포함되지 않습니다.

### THRUNT 업데이트가 로컬 변경사항을 덮어쓴 경우

v1.17부터 설치 프로그램이 로컬로 수정된 파일을 `thrunt-local-patches/`에 백업합니다. 변경사항을 다시 병합하려면 `/thrunt:reapply-patches`를 실행하세요.

### 워크플로우 진단 (`/thrunt:forensics`)

워크플로우가 명확하지 않은 방식으로 실패할 때 — 계획이 존재하지 않는 파일을 참조하거나 실행이 예상치 못한 결과를 생성하거나 상태가 손상된 것 같을 때 — `/thrunt:forensics`를 실행하여 진단 보고서를 생성하세요.

**검사 항목.**
- Git 히스토리 이상 (고아 커밋, 예상치 못한 브랜치 상태, rebase 아티팩트)
- 아티팩트 무결성 (누락되거나 잘못된 계획 파일, 끊어진 교차 참조)
- 상태 불일치 (실제 파일 존재 여부 대비 HUNTMAP 상태, 설정 드리프트)

**출력:** 발견사항과 권장 수정 단계가 포함된 `.planning/forensics/`의 진단 보고서.

### 서브에이전트가 실패한 것 같지만 작업이 완료된 경우

Claude Code 분류 버그에 대한 알려진 해결 방법이 있습니다. THRUNT의 오케스트레이터 (hunt-run, quick)는 실패를 보고하기 전에 실제 출력을 현장 확인합니다. 실패 메시지가 표시되었지만 커밋이 이루어진 경우 `git log`를 확인하세요 — 작업이 성공했을 수 있습니다.

### 병렬 실행으로 인한 빌드 잠금 오류

병렬 웨이브 실행 중에 pre-commit 훅 실패, cargo lock 경합, 또는 30분 이상의 실행 시간이 발생한다면 여러 에이전트가 동시에 빌드 도구를 실행하기 때문입니다. THRUNT는 v1.26부터 이를 자동으로 처리합니다 — 병렬 에이전트는 커밋에 `--no-verify`를 사용하고 오케스트레이터가 각 웨이브 후 한 번 훅을 실행합니다. 이전 버전을 사용하는 경우 프로젝트의 `CLAUDE.md`에 다음을 추가하세요.
...
병렬 실행을 완전히 비활성화하려면: `/thrunt:settings` → `parallelization.enabled`를 `false`로 설정합니다.
...
| 문제 | 해결 방법 |
|------|----------|
| 컨텍스트 손실 / 새 세션 | `/thrunt:resume-work` 또는 `/thrunt:progress` |
| 페이즈가 잘못됨 | 페이즈 커밋에 `git revert` 후 재계획 |
| 범위 변경 필요 | `/thrunt:add-phase`, `/thrunt:insert-phase`, 또는 `/thrunt:remove-phase` |
| 마일스톤 감사에서 갭 발견 | `/thrunt:plan-milestone-gaps` |
| 무언가 고장남 | `/thrunt:debug "description"` |
| 워크플로우 상태 손상 의심 | `/thrunt:forensics` |
| 빠른 목표 수정 | `/thrunt:quick` |
| 계획이 비전과 맞지 않음 | `/hunt:shape-hypothesis [N]` 후 재계획 |
| 비용이 높아짐 | `/thrunt:set-profile budget` 및 `/thrunt:settings`에서 에이전트 비활성화 |
| 업데이트가 로컬 변경사항 파괴 | `/thrunt:reapply-patches` |
| 이해관계자를 위한 세션 요약 필요 | `/thrunt:session-report` |
| 다음 단계를 모르겠음 | `/thrunt:next` |
| 병렬 실행 빌드 오류 | THRUNT 업데이트 또는 `parallelization.enabled: false` 설정 |

---

## 프로젝트 파일 구조

참고로 THRUNT가 프로젝트에 생성하는 파일 구조입니다.
