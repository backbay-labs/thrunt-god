...
THRUNT는 멀티 에이전트 아키텍처를 사용합니다. 가벼운 오케스트레이터(워크플로우 파일)가 전문화된 에이전트를 새로운 컨텍스트 윈도우로 생성합니다. 각 에이전트는 집중된 역할, 제한된 도구 접근 권한을 가지며 특정 결과물을 생성합니다.
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/hunt:new-program`, `/hunt:new-program` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `shape-assumptions` 워크플로우 (`workflow.discuss_mode = 'assumptions'`인 경우) |
| **병렬성** | 단일 인스턴스 |
...
**핵심 동작.**
- HUNTMAP.md 단계 설명과 이전 CONTEXT.md 파일을 읽습니다
...
**역할:** shape-hypothesis 어드바이저 모드에서 단일 회색 지대 결정을 조사하고 구조화된 비교 표를 반환합니다.

| 속성 | 값 |
|----------|-------|
| **생성 주체** | `shape-hypothesis` 워크플로우 (ADVISOR_MODE = true인 경우) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/hunt:new-program` (4개 조사자 완료 후) |
...
**핵심 동작.**
- MISSION.md, HYPOTHESES.md, CONTEXT.md, RESEARCH.md를 읽습니다
...
**역할:** 원자적 커밋, 이탈 처리, 체크포인트 프로토콜로 THRUNT 계획을 실행합니다.

| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/hunt:run`, `/thrunt:quick` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/hunt:plan` (검증 루프, 최대 3회 반복) |
| **병렬성** | 단일 인스턴스 (반복적) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/thrunt:ui-phase` (검증 루프, 최대 2회 반복) |
| **병렬성** | 단일 인스턴스 |
| **도구** | Read, Bash, Glob, Grep |
| **모델 (balanced)** | Sonnet |
| **색상** | `#22D3EE` (cyan) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/thrunt:debug`, `/hunt:validate-findings` (실패 시) |
| **병렬성** | 단일 인스턴스 (대화형) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/thrunt:profile-user` |
| **병렬성** | 단일 인스턴스 |
| **도구** | Read |
| **모델 (balanced)** | Sonnet |
| **색상** | Magenta |
| **생성물** | `USER-PROFILE.md`, `/thrunt:dev-preferences`, `CLAUDE.md` 프로필 섹션 |
