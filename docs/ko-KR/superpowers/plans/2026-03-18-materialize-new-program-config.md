# 초기화 시 new-program config 완전 구체화
...
**목표:** `/hunt:new-program`가 `.planning/config.json`을 생성할 때, 파일에 사용자가 선택한 6개 키만이 아닌 모든 유효한 기본값이 포함되도록 하여 개발자가 소스 코드를 읽지 않고도 모든 설정을 확인할 수 있게 합니다.

**아키텍처:** 새 프로젝트의 전체 config에 대한 단일 진실 공급원으로서 `config.cjs`에 단일 JS 함수 `buildNewProgramConfig(cwd, userChoices)`를 추가합니다. CLI 명령어 `config-new-program`로 노출합니다. 부분적인 JSON을 인라인으로 작성하는 대신 이 명령어를 호출하도록 `hunt-bootstrap.md` 워크플로우를 업데이트합니다.

**기술 스택:** Node.js/CommonJS, 기존 thrunt-tools CLI, 테스트에는 `node:test`.

---

## 배경: 현재 상태

`hunt-bootstrap.md` Step 5는 이 부분적인 config를 작성합니다(AI가 템플릿을 채움):
...
| 파일 | 액션 | 목적 |
|------|--------|---------|
| `thrunt-god/bin/lib/config.cjs` | 수정 | `buildNewProgramConfig()` + `cmdConfigNewProgram()` 추가 |
| `thrunt-god/bin/thrunt-tools.cjs` | 수정 | `config-new-program` case 등록 + usage 문자열 업데이트 |
| `thrunt-god/workflows/hunt-bootstrap.md` | 수정 | Steps 2a + 5: 인라인 JSON 작성을 CLI 호출로 교체 |
| `tests/config.test.cjs` | 수정 | `config-new-program` 테스트 스위트 추가 |

---

## 작업 1: config.cjs에 `buildNewProgramConfig`와 `cmdConfigNewProgram` 추가

**파일.**

- 수정: `thrunt-god/bin/lib/config.cjs`
...
예상 결과: 모든 `config-new-program` 테스트가 "config-new-program is not a valid command" 또는 유사한 오류로 실패합니다.

- [ ] **Step 1.3: config.cjs에 `buildNewProgramConfig`와 `cmdConfigNewProgram` 구현**

`thrunt-god/bin/lib/config.cjs`에서 `validateKnownConfigKeyPath` 함수 뒤(약 35번째 줄)와 `ensureConfigFile` 앞에 다음을 추가하세요:
 ...
 * 다음 우선순위 순서로 병합합니다:
 *   1. 하드코딩된 기본값
 *   2. ~/.thrunt/defaults.json의 사용자 수준 기본값(있는 경우)
 *   3. userChoices (new-program 중 사용자가 명시적으로 선택한 설정)
 *
...
function buildNewProgramConfig(cwd, userChoices) {
  ...
  // 사용 가능한 경우 ~/.thrunt/defaults.json에서 사용자 수준 기본값 로드
 ...
 *
 * 사용자가 선택한 설정을 JSON 문자열로 받습니다(/hunt:new-program 중 명시적으로
 * 구성한 키들). 나머지 키들은 하드코딩된 기본값과 선택적 ~/.thrunt/defaults.json에서 채워집니다.
 *
...
`config.cjs` 하단의 `module.exports`에 `cmdConfigNewProgram`도 추가하세요.
...
예상 결과: 모든 `config-new-program` 테스트가 통과합니다. 기존 테스트도 계속 통과합니다.
...
## 작업 2: thrunt-tools.cjs에 `config-new-program` 등록

**파일.**

- 수정: `thrunt-god/bin/thrunt-tools.cjs`

- [ ] **Step 2.1: thrunt-tools.cjs의 switch에 case 추가**
...
178번째 줄의 usage 문자열도 `config-new-program`를 포함하도록 업데이트하세요:

현재: `...config-ensure-section, init`
변경: `...config-ensure-section, config-new-program, init`
...
## 작업 3: config-new-program를 사용하도록 hunt-bootstrap.md 워크플로우 업데이트

**파일.**

- 수정: `thrunt-god/workflows/hunt-bootstrap.md`
...
예상 결과: `config-new-program` 2회 등장(단계당 하나씩), config 생성을 위한 인라인 JSON 템플릿 없음.
...
`hunt-bootstrap.md`가 새 프로젝트에서 수행하는 작업을 시뮬레이션합니다:
...
# Step 1 시뮬레이션: init new-program가 반환하는 내용
...
**문제:**
`/hunt:new-program`는 온보딩 중 사용자가 명시적으로 선택한 6개 키만으로
...
2. **암묵적 확장**: `/thrunt:settings` 또는 `config-set`이 처음으로 config에
   기록할 때도 해당 키들이 추가되지 않습니다. config는 유효한 구성의
   일부만 반영합니다.

**해결책:**
`thrunt-tools.cjs`에 `config-new-program` CLI 명령어를 추가합니다. 이 명령어는:
...
`hunt-bootstrap.md` 워크플로우(Steps 2a와 5)를 업데이트합니다. 기본값은 이제
정확히 한 곳에 있습니다: `config.cjs`의 `buildNewProgramConfig()`.
...
- 멱등성: `config-new-program`를 두 번 호출해도 안전합니다.
- 새로운 사용자 대면 플래그 없음

**이것이 발견성을 개선하는 이유:**
처음으로 `.planning/config.json`을 여는 개발자는 이제
`git.branching_strategy: "none"`을 확인하고 THRUNT 소스를 읽지 않고도
