...
**사전 조건:** `.planning/MISSION.md`가 존재하지 않아야 합니다.
**생성 파일:** `MISSION.md`, `HYPOTHESES.md`, `HUNTMAP.md`, `STATE.md`, `config.json`, `research/`, `CLAUDE.md`

```bash
/hunt:new-program                    # 대화형 모드
/hunt:new-program --auto @prd.md     # PRD에서 자동 추출
```
...
/thrunt:new-workspace --name feature-b --repos . --strategy worktree  # 동일 저장소 격리
/thrunt:new-workspace --name spike --repos api,web --strategy clone   # 전체 클론
```
...
활성 THRUNT 워크스페이스와 상태를 목록으로 표시합니다.

**스캔 위치:** `~/thrunt-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
**표시 항목:** 이름, 저장소 수, 전략, THRUNT 프로젝트 상태
...
**사전 조건:** `.planning/HUNTMAP.md`가 존재해야 합니다.
**생성 파일:** `{phase}-CONTEXT.md`, `{phase}-DISCUSSION-LOG.md` (감사 추적)

```bash
/hunt:shape-hypothesis 1                # 페이즈 1 대화형 토론
/hunt:shape-hypothesis 3 --auto         # 페이즈 3 기본값 자동 선택
/hunt:shape-hypothesis --batch          # 현재 페이즈 일괄 모드
/hunt:shape-hypothesis 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
**사전 조건:** `.planning/HUNTMAP.md`가 존재해야 하며 해당 페이즈에 프론트엔드/UI 작업이 포함되어야 합니다.
**생성 파일:** `{phase}-UI-SPEC.md`

```bash
/thrunt:ui-phase 2                     # 페이즈 2 설계 계약 생성
```
...
| `--gaps` | 갭 보완 모드 (FINDINGS.md를 읽고 조사를 건너뜁니다) |
| `--skip-verify` | 계획 검증 루프를 건너뜁니다 |
| `--prd <file>` | shape-hypothesis 대신 PRD 파일을 컨텍스트로 사용합니다 |
...
```bash
/hunt:plan 1                   # 페이즈 1 조사 + 계획 + 검증
/hunt:plan 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
/hunt:plan --auto              # 비대화형 계획 수립
```
...
**생성 파일:** 계획별 `{phase}-{N}-SUMMARY.md`, git 커밋, 페이즈가 완전히 완료되면 `{phase}-FINDINGS.md`

```bash
/hunt:run 1                # 페이즈 1 실행
/hunt:run 1 --wave 2       # Wave 2만 실행
```
...
자동 진단을 포함한 사용자 인수 테스트(Evidence Review)를 수행합니다.
...
**생성 파일:** `{phase}-EVIDENCE_REVIEW.md`, 문제 발견 시 수정 계획

```bash
/hunt:validate-findings 1                  # 페이즈 1 Evidence Review
```
...
**동작 방식.**
- 프로젝트 없음 → `/hunt:new-program` 제안
- 페이즈 토론 필요 → `/hunt:shape-hypothesis` 실행
- 페이즈 계획 필요 → `/hunt:plan` 실행
- 페이즈 실행 필요 → `/hunt:run` 실행
- 페이즈 검증 필요 → `/hunt:validate-findings` 실행
...
```bash
/thrunt:session-report                 # 세션 종료 후 요약 생성
```
...
**사전 조건:** 페이즈 검증 완료 (`/hunt:validate-findings` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/hunt:publish 4                         # 페이즈 4 출시
/hunt:publish 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, THRUNT 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`에 스크린샷

```bash
/thrunt:ui-review                      # 현재 페이즈 감사
/thrunt:ui-review 3                    # 페이즈 3 감사
```
...
모든 미완료 Evidence Review 및 검증 항목에 대한 교차 페이즈 감사를 수행합니다.

**사전 조건:** Evidence Review 또는 검증이 포함된 페이즈가 하나 이상 실행되어 있어야 합니다.
...
```bash
/thrunt:milestone-summary                # 현재 마일스톤 요약
/thrunt:milestone-summary v1.0           # 특정 마일스톤 요약
```
...
**생성 파일:** 업데이트된 `MISSION.md`, 새 `HYPOTHESES.md`, 새 `HUNTMAP.md`

```bash
/hunt:new-program                  # 대화형 모드
/hunt:new-program "v2.0 Mobile"    # 이름이 지정된 마일스톤
/hunt:new-program --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/thrunt:add-phase                      # 대화형 — 페이즈를 설명합니다
```
...
```bash
/thrunt:insert-phase 3                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/thrunt:remove-phase 7                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/thrunt:list-phase-assumptions 2       # 페이즈 2 가정 사항 확인
```
...
```bash
/thrunt:plan-milestone-gaps             # 각 감사 갭에 대한 페이즈 생성
```

### `/hunt:shape-hypothesis`

심층 에코시스템 조사만 수행합니다 (독립 실행 — 일반적으로 `/hunt:plan`를 사용하세요).
...
```bash
/hunt:shape-hypothesis 4               # 페이즈 4 도메인 조사
```
...
```bash
/thrunt:validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/thrunt:progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/thrunt:resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/thrunt:pause-work                     # continue-here.md 생성
```
...
```bash
/thrunt:quick                          # 기본 빠른 작업
/thrunt:quick --discuss --research     # 토론 + 조사 + 계획
/thrunt:quick --full                   # 계획 검사 및 결과 검증 포함
/thrunt:quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/thrunt:autonomous                     # 남은 모든 페이즈 실행
/thrunt:autonomous --from 3            # 페이즈 3부터 시작
```

### `/thrunt:do`

자유 형식 텍스트를 적절한 THRUNT 명령어로 라우팅합니다.

```bash
/thrunt:do                             # 원하는 작업을 설명합니다
```
...
```bash
/thrunt:add-tests 2                    # 페이즈 2 테스트 생성
```
...
- `/thrunt:dev-preferences` 명령어 — 모든 세션에서 선호도를 로드합니다
- `CLAUDE.md` 프로필 섹션 — Claude Code에 의해 자동으로 인식됩니다

```bash
/thrunt:profile-user                   # 세션 분석 및 프로필 구축
/thrunt:profile-user --questionnaire   # 대화형 설문지 대체 방법
/thrunt:profile-user --refresh         # 새로운 분석으로 재생성
```
...
```bash
/thrunt:health                         # 무결성 검사
/thrunt:health --repair                # 검사 및 수정
```
...
실패하거나 멈춘 THRUNT 워크플로우에 대한 사후 조사를 수행합니다.
...
```bash
/thrunt:forensics                              # 대화형 — 문제 입력 프롬프트
/thrunt:forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
**사전 조건:** 활성 THRUNT 프로젝트
**생성 파일:** `.planning/` 하위의 워크스트림 디렉터리, 워크스트림별 상태 추적

```bash
/thrunt:workstreams                    # 모든 워크스트림 목록
/thrunt:workstreams create backend-api # 새 워크스트림 생성
/thrunt:workstreams switch backend-api # 활성 워크스트림 설정
/thrunt:workstreams status backend-api # 상세 상태 확인
/thrunt:workstreams progress           # 교차 워크스트림 진행 상황 개요
/thrunt:workstreams complete backend-api  # 완료된 워크스트림 아카이브
/thrunt:workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/thrunt:set-profile budget             # 예산 프로필로 전환
/thrunt:set-profile quality            # 품질 프로필로 전환
```
...
```bash
/hunt:map-environment                   # 전체 코드베이스 분석
/hunt:map-environment auth              # 인증 영역에 집중
```
...
변경 로그 미리보기와 함께 THRUNT를 업데이트합니다.

```bash
/thrunt:update                         # 업데이트 확인 및 설치
```

### `/thrunt:reapply-patches`

THRUNT 업데이트 후 로컬 수정사항을 복원합니다.

```bash
/thrunt:reapply-patches                # 로컬 변경사항 병합
```
...
**`/thrunt:quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/thrunt:quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/hunt:plan --reviews`에서 사용 가능
...
**목적:** 리뷰어에게 THRUNT 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/thrunt:pr-branch                     # main을 기준으로 필터링
/thrunt:pr-branch develop             # develop을 기준으로 필터링
```
...
**999.x 번호 체계**는 백로그 항목을 활성 페이즈 순서 밖에 유지합니다. 페이즈 디렉터리가 즉시 생성되므로 해당 항목에 대해 `/hunt:shape-hypothesis`와 `/hunt:plan`를 사용할 수 있습니다.
...
**사용처:** `/hunt:new-program` (시드를 스캔하여 일치 항목 제시)
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/thrunt:pause-work`보다 가볍습니다.

```bash
/thrunt:thread                         # 모든 스레드 목록
/thrunt:thread fix-deploy-key-auth     # 스레드 재개
/thrunt:thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
