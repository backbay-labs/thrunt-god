# Multi-Project Workspaces (`/thrunt:new-workspace`)
...
THRUNT is tied to one `.planning/` directory per working directory. Users with multiple independent projects (monorepo-style setups with 20+ child repos) or users needing feature branch isolation in the same repo cannot run parallel THRUNT sessions without manual cloning and state management.
...
```
/thrunt:new-workspace --name feature-b --repos hr-ui,ZeymoAPI --path ~/workspaces/feature-b
...
| `--path` | No | `~/thrunt-workspaces/<name>` | Target directory |
...
Scans `~/thrunt-workspaces/*/WORKSPACE.md` for workspace manifests. Displays table with name, path, repo count, THRUNT status (has MISSION.md, current phase).
...
```
~/thrunt-workspaces/feature-b/          # workspace root
├── WORKSPACE.md                      # manifest
├── .planning/                        # independent THRUNT planning directory
│   ├── MISSION.md                    # (if user ran /hunt:new-program)
│   ├── STATE.md
│   └── config.json
...
- `WORKSPACE.md` is the only THRUNT-specific file at the root (besides `.planning/`)
...
### `/thrunt:new-workspace` Workflow Steps
...
8. **Offer /hunt:new-program** — Ask if user wants to run project initialization in the new workspace
...
- Default workspace base dir (`~/thrunt-workspaces/`)
...
- **`~/thrunt-workspaces/` doesn't exist** — "No workspaces found"
...
| Component | Path |
|-----------|------|
| Command: new-workspace | `commands/thrunt/new-workspace.md` |
| Command: list-workspaces | `commands/thrunt/list-workspaces.md` |
| Command: remove-workspace | `commands/thrunt/remove-workspace.md` |
| Workflow: new-workspace | `thrunt-god/workflows/new-workspace.md` |
| Workflow: list-workspaces | `thrunt-god/workflows/list-workspaces.md` |
| Workflow: remove-workspace | `thrunt-god/workflows/remove-workspace.md` |
| Init function | `thrunt-god/bin/lib/init.cjs` (add `cmdInitNewWorkspace`, `cmdInitListWorkspaces`, `cmdInitRemoveWorkspace`) |
| Routing | `thrunt-god/bin/thrunt-tools.cjs` (add cases to init switch) |
...
| Decision | Rationale |
|----------|-----------|
| Physical directories over logical registry | Filesystem is source of truth — matches THRUNT's existing cwd-based detection pattern |
| Worktree as default strategy | Lightweight (shared .git objects), fast to create, easy to clean up |
| `.planning/` at workspace root | Gives full isolation from individual repo planning. Each workspace is an independent THRUNT project |
...
| Default path `~/thrunt-workspaces/<name>` | Predictable location for `list-workspaces` to scan, keeps workspaces out of source repos |
