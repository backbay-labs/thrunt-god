# Materialize new-program config on initialization

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** When `/hunt:new-program` creates `.planning/config.json`, the file contains all effective defaults — not just the 6 user-chosen keys — so developers can see every setting without reading source code.

**Architecture:** Add a single JS function `buildNewProgramConfig(cwd, userChoices)` in `config.cjs` as the one source of truth for a new project's full config. Expose it as a CLI command `config-new-program`. Update the `hunt-bootstrap.md` workflow to call this command instead of writing a partial JSON inline.

**Tech Stack:** Node.js/CommonJS, existing thrunt-tools CLI, `node:test` for tests.

---

## Background: what exists today

`hunt-bootstrap.md` Step 5 writes this partial config (the AI fills the template):
...
| File | Action | Purpose |
|------|--------|---------|
| `thrunt-god/bin/lib/config.cjs` | Modify | Add `buildNewProgramConfig()` + `cmdConfigNewProgram()` |
| `thrunt-god/bin/thrunt-tools.cjs` | Modify | Register `config-new-program` case + update usage string |
| `thrunt-god/workflows/hunt-bootstrap.md` | Modify | Steps 2a + 5: replace inline JSON write with CLI call |
| `tests/config.test.cjs` | Modify | Add `config-new-program` test suite |

---

## Task 1: Add `buildNewProgramConfig` and `cmdConfigNewProgram` to config.cjs

**Files:**

- Modify: `thrunt-god/bin/lib/config.cjs`
...
Expected: All `config-new-program` tests fail with "config-new-program is not a valid command" or similar.

- [ ] **Step 1.3: Implement `buildNewProgramConfig` and `cmdConfigNewProgram` in config.cjs**

In `thrunt-god/bin/lib/config.cjs`, add the following after the `validateKnownConfigKeyPath` function (around line 35) and before `ensureConfigFile`:
 ...
 *   3. userChoices (the settings the user explicitly selected during new-program)
 *
...
Also add `cmdConfigNewProgram` to the `module.exports` at the bottom of `config.cjs`.
...
Expected: All `config-new-program` tests pass. Existing tests still pass.
...
## Task 2: Register `config-new-program` in thrunt-tools.cjs

**Files:**

- Modify: `thrunt-god/bin/thrunt-tools.cjs`

- [ ] **Step 2.1: Add the case to the switch in thrunt-tools.cjs**
...
Also update the usage string on line 178 to include `config-new-program`:

Current: `...config-ensure-section, init`
New: `...config-ensure-section, config-new-program, init`
...
Clean up: `rm -rf /tmp/thrunt-smoke-*`
...
## Task 3: Update hunt-bootstrap.md workflow to use config-new-program

**Files:**

- Modify: `thrunt-god/workflows/hunt-bootstrap.md`
...
Expected: 2 occurrences of `config-new-program` (one per step), no more inline JSON templates for config creation.
...
Simulate what `hunt-bootstrap.md` does for a new project:
...
# Step 1 simulation: what init new-program returns
...
**Problem:**
`/hunt:new-program` creates `.planning/config.json` with only the 6 keys
...
2. **Implicit expansion**: the first time `/thrunt:settings` or `config-set`
   writes to the config, those keys still aren't added. The config only
   reflects a fraction of the effective configuration.

**Solution:**
Add `config-new-program` CLI command to `thrunt-tools.cjs`. The command:
...
Update `hunt-bootstrap.md` workflow (Steps 2a and 5) to call this command
instead of writing a hardcoded partial JSON template. Defaults now live in
exactly one place: `buildNewProgramConfig()` in `config.cjs`.
...
- Fully compatible: `loadConfig()` continues to handle both the old
  partial format (existing projects) and the new full format
- Idempotent: calling `config-new-program` twice is safe
...
is available and configurable, without reading the THRUNT source.
```
