...
所以我开发了 THRUNT。复杂性在系统内部，不在你的工作流里。幕后是：上下文工程、XML 提示格式、子代理编排、状态管理。你看到的是：几个命令，用就完了。
...
THRUNT 解决了这个问题。它是让 Claude Code 变得可靠的上下文工程层。描述你的想法，让系统提取它需要知道的一切，然后让 Claude Code 开始工作。
...
> [!NOTE]
> Codex 安装使用技能（`skills/thrunt-*/SKILL.md`）而非自定义提示。
...
# 所有运行时
npx thrunt-god --all --global      # 安装到所有目录
```
...
THRUNT 设计为无摩擦自动化。运行 Claude Code 时使用：
...
> [!TIP]
> 这是 THRUNT 的预期使用方式 —— 停下来 50 次批准 `date` 和 `git commit` 会失去意义。
...
> **已有代码？** 先运行 `/hunt:map-environment`。它会生成并行代理分析你的技术栈、架构、约定和关注点。然后 `/hunt:new-program` 就了解你的代码库了 —— 问题聚焦在你正在**添加**什么，规划会自动加载你的模式。
...
**创建：** `MISSION.md`、`HYPOTHESES.md`、`HUNTMAP.md`、`STATE.md`、`.planning/research/`
...
**创建：** `{阶段号}-{N}-SUMMARY.md`、`{阶段号}-FINDINGS.md`
...
如果一切通过，继续。如果有东西坏了，不用手动调试 —— 只需再次运行 `/hunt:run`，使用它创建的修复计划。

**创建：** `{阶段号}-EVIDENCE_REVIEW.md`，如果发现问题则创建修复计划
...
如果你想在讨论期间更快速地输入，使用 `/hunt:shape-hypothesis <n> --batch` 一次回答一组小问题，而不是一个一个来。

每个阶段都会获得你的输入（讨论）、适当的研究（规划）、干净的执行（执行）和人工验证（验证）。上下文保持新鲜。质量保持高水平。

当所有阶段完成后，`/thrunt:complete-milestone` 归档里程碑并标记发布。

然后 `/hunt:new-program` 开始下一个版本 —— 与 `new-program` 相同的流程，但针对你现有的代码库。你描述接下来想构建什么，系统研究领域，你界定需求范围，它创建新的路线图。每个里程碑是一个干净的周期：定义 → 构建 → 发布。
...
快速模式给你 THRUNT 保证（原子提交、状态跟踪）和更快的路径：
...
| `HYPOTHESES.md` | 界定 v1/v2 需求及阶段可追溯性 |
| `HUNTMAP.md` | 你要去哪里，完成了什么 |
...
| `/hunt:shape-hypothesis [N] [--auto]` | 在规划前捕获实现决策 |
| `/hunt:plan [N] [--auto]` | 阶段的研究 + 规划 + 验证 |
| `/hunt:run <N>` | 在并行波次中执行所有计划，完成后验证 |
| `/hunt:validate-findings [N]` | 手动用户验收测试 ¹ |
| `/thrunt:audit-milestone` | 验证里程碑达到了其完成定义 |
| `/thrunt:complete-milestone` | 归档里程碑，标记发布 |
...
| 命令 | 作用 |
|---------|--------------|
| `/thrunt:progress` | 我在哪？接下来做什么？ |
| `/thrunt:help` | 显示所有命令和使用指南 |
...
| 命令 | 作用 |
|---------|--------------|
| `/hunt:map-environment` | 在 new-program 之前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|---------|--------------|
| `/thrunt:add-phase` | 向路线图追加阶段 |
| `/thrunt:insert-phase [N]` | 在阶段之间插入紧急工作 |
| `/thrunt:remove-phase [N]` | 删除未来阶段，重新编号 |
| `/thrunt:list-phase-assumptions [N]` | 规划前查看 Claude 的预期方法 |
| `/thrunt:plan-milestone-gaps` | 创建阶段以填补审计发现的差距 |

### 会话

| 命令 | 作用 |
|---------|--------------|
| `/thrunt:pause-work` | 阶段中途停止时创建交接 |
| `/thrunt:resume-work` | 从上次会话恢复 |

### 工具

| 命令 | 作用 |
|---------|--------------|
| `/thrunt:settings` | 配置模型配置文件和工作流代理 |
| `/thrunt:set-profile <profile>` | 切换模型配置文件（quality/balanced/budget） |
| `/thrunt:add-todo [desc]` | 捕获想法留待后用 |
| `/thrunt:check-todos` | 列出待处理事项 |
| `/thrunt:debug [desc]` | 带持久状态的系统化调试 |
| `/thrunt:quick [--full] [--discuss]` | 用 THRUNT 保证执行临时任务（`--full` 添加计划检查和验证，`--discuss` 先收集上下文） |
| `/thrunt:health [--repair]` | 验证 `.planning/` 目录完整性，用 `--repair` 自动修复 |
...
THRUNT 在 `.planning/config.json` 中存储项目设置。在 `/hunt:new-program` 期间配置或稍后用 `/thrunt:settings` 更新。完整配置模式、工作流开关、git 分支选项和每个代理的模型分解，请参阅[用户指南](USER-GUIDE.md#配置参考)。
...
| `workflow.validator` | `true` | 执行后确认必须项已交付 |
| `workflow.auto_advance` | `false` | 自动链式执行 讨论 → 规划 → 执行 |

使用 `/thrunt:settings` 切换这些，或每次调用时覆盖：
...
控制 THRUNT 在执行期间如何处理分支。

| 设置 | 选项 | 默认值 | 作用 |
|---------|---------|---------|--------------|
| `git.branching_strategy` | `none`, `phase`, `milestone` | `none` | 分支创建策略 |
| `git.phase_branch_template` | 字符串 | `thrunt/phase-{phase}-{slug}` | 阶段分支模板 |
| `git.milestone_branch_template` | 字符串 | `thrunt/{milestone}-{slug}` | 里程碑分支模板 |

**策略：**
- **`none`** —— 提交到当前分支（默认 THRUNT 行为）
- **`phase`** —— 每个阶段创建一个分支，阶段完成时合并
- **`milestone`** —— 为整个里程碑创建一个分支，完成时合并

在里程碑完成时，THRUNT 提供 squash 合并（推荐）或带历史合并。
...
THRUNT 的代码库映射和分析命令读取文件以了解你的项目。**保护包含密钥的文件**，将它们添加到 Claude Code 的拒绝列表：
...
> [!IMPORTANT]
> THRUNT 包含内置保护以防止提交密钥，但纵深防御是最佳实践。拒绝读取敏感文件作为第一道防线。
...
**安装后找不到命令？**
- 重启运行时以重新加载命令/技能
- 验证文件是否存在于 `~/.claude/commands/thrunt/`（全局）或 `./.claude/commands/thrunt/`（本地）
- 对于 Codex，验证技能是否存在于 `~/.codex/skills/thrunt-*/SKILL.md`（全局）或 `./.codex/skills/thrunt-*/SKILL.md`（本地）

**命令没有按预期工作？**
- 运行 `/thrunt:help` 验证安装
- 重新运行 `npx thrunt-god` 重新安装
...
这删除所有 THRUNT 命令、代理、钩子和设置，同时保留你的其他配置。

---

## 社区移植

OpenCode、Gemini CLI 和 Codex 现在通过 `npx thrunt-god` 原生支持。

这些社区移植开创了多运行时支持：

| 项目 | 平台 | 描述 |
|---------|----------|-------------|
| [thrunt-opencode](https://github.com/rokicool/thrunt-opencode) | OpenCode | 原始 OpenCode 适配 |
| thrunt-gemini (已归档) | Gemini CLI | 由 uberfuzzy 开发的原始 Gemini 适配 |
