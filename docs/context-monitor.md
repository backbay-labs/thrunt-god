...
| CRITICAL | <= 25% | Stop immediately, save state (`/thrunt:pause-work`) |
...
```
Statusline Hook (thrunt-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (thrunt-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
## Integration with THRUNT

THRUNT's `/thrunt:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx thrunt-god` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/thrunt-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/thrunt-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/thrunt-context-monitor.js"
          }
        ]
      }
    ]
