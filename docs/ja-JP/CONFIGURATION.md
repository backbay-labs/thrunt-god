...
THRUNT はプロジェクト設定を `.planning/config.json` に保存します。`/hunt:new-program` 実行時に作成され、`/thrunt:settings` で更新できます。
...
| `workflow.validator` | boolean | `true` | 実行後にフェーズ目標に対する検証を実施 |
| `workflow.auto_advance` | boolean | `false` | discuss → plan → execute を停止せずに自動連鎖 |
| `workflow.nyquist_validation` | boolean | `true` | hunt-plan のリサーチ中にテストカバレッジマッピングを実施 |
| `workflow.ui_phase` | boolean | `true` | フロントエンドフェーズで UI デザインコントラクトを生成 |
| `workflow.ui_safety_gate` | boolean | `true` | hunt-plan 中にフロントエンドフェーズに対して /thrunt:ui-phase の実行を促すプロンプトを表示 |
...
| `workflow.discuss_mode` | string | `'discuss'` | `/hunt:shape-hypothesis` のコンテキスト収集方法を制御。`'discuss'`（デフォルト）は質問を1つずつ行います。`'assumptions'` はまずコードベースを読み取り、信頼度レベル付きの構造化された仮説を生成し、誤っている点のみ修正を求めます。v1.28 で追加 |
| `workflow.skip_discuss` | boolean | `false` | `true` の場合、`/thrunt:autonomous` は shape-hypothesis を完全にスキップし、HUNTMAP のフェーズ目標から最小限の CONTEXT.md を作成します。開発者の要望が MISSION.md/HYPOTHESES.md に十分に記載されているプロジェクトに適しています。v1.28 で追加 |
| `workflow.text_mode` | boolean | `false` | AskUserQuestion の TUI メニューをプレーンテキストの番号付きリストに置き換えます。TUI メニューが表示されない Claude Code リモートセッション（`/rc` モード）で必要です。shape-hypothesis で `--text` フラグを使用してセッションごとに設定することもできます。v1.28 で追加 |

### 推奨プリセット

| シナリオ | mode | granularity | profile | research | plan_check | validator |
|---------|------|-------------|---------|----------|------------|----------|
...
| `hooks.workflow_guard` | boolean | `false` | THRUNT ワークフローのコンテキスト外でファイル編集が行われた場合に警告（`/thrunt:quick` または `/thrunt:fast` の使用を推奨） |

プロンプトインジェクションガードフック（`thrunt-prompt-guard.js`）は常に有効であり、無効にすることはできません。これはワークフロートグルではなく、セキュリティ機能です。
...
| `git.phase_branch_template` | string | `thrunt/phase-{phase}-{slug}` | phase 戦略のブランチ名テンプレート |
| `git.milestone_branch_template` | string | `thrunt/{milestone}-{slug}` | milestone 戦略のブランチ名テンプレート |
| `git.quick_branch_template` | string or null | `null` | `/thrunt:quick` タスク用のオプションのブランチ名テンプレート |
...
| `phase` | `hunt-run` 開始時 | 1フェーズ | フェーズ後にユーザーがマージ | フェーズごとのコードレビュー、きめ細かいロールバック |
| `milestone` | 最初の `hunt-run` 時 | マイルストーン内の全フェーズ | `complete-milestone` 時 | リリースブランチ、バージョンごとの PR |
...
| `gates.confirm_huntmap` | boolean | `true` | 続行前にロードマップを確認 |
...
THRUNT が非 Claude ランタイム向けにインストールされると、インストーラーは自動的に `~/.thrunt/defaults.json` に `resolve_model_ids: "omit"` を設定します。これにより THRUNT はすべてのエージェントに対して空のモデルパラメータを返し、各エージェントはランタイムで設定されたモデルを使用します。デフォルトの場合、追加のセットアップは不要です。
...
**保存場所:** `~/.thrunt/defaults.json`

`/hunt:new-program` が新しい `config.json` を作成する際、グローバルデフォルトを読み込み、初期設定としてマージします。プロジェクトごとの設定は常にグローバル設定を上書きします。
