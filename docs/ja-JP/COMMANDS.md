...
**前提条件:** 既存の `.planning/MISSION.md` がないこと
**生成物:** `MISSION.md`、`HYPOTHESES.md`、`HUNTMAP.md`、`STATE.md`、`config.json`、`research/`、`CLAUDE.md`

```bash
/hunt:new-program                    # 対話モード
/hunt:new-program --auto @prd.md     # PRDから自動抽出
```
...
/thrunt:new-workspace --name feature-b --repos . --strategy worktree  # 同一リポジトリの分離
/thrunt:new-workspace --name spike --repos api,web --strategy clone   # フルクローン
```
...
アクティブなTHRUNTワークスペースとそのステータスを一覧表示します。

**スキャン対象:** `~/thrunt-workspaces/` 内の `WORKSPACE.md` マニフェスト
**表示内容:** 名前、リポジトリ数、戦略、THRUNTプロジェクトのステータス
...
**前提条件:** `.planning/HUNTMAP.md` が存在すること
**生成物:** `{phase}-CONTEXT.md`、`{phase}-DISCUSSION-LOG.md`（監査証跡）

```bash
/hunt:shape-hypothesis 1                # フェーズ1の対話的ディスカッション
/hunt:shape-hypothesis 3 --auto         # フェーズ3でデフォルトを自動選択
/hunt:shape-hypothesis --batch          # 現在のフェーズのバッチモード
/hunt:shape-hypothesis 2 --analyze      # トレードオフ分析付きディスカッション
```
...
**前提条件:** `.planning/HUNTMAP.md` が存在し、フェーズにフロントエンド/UI作業があること
**生成物:** `{phase}-UI-SPEC.md`

```bash
/thrunt:ui-phase 2                     # フェーズ2のデザイン契約書
```
...
| `--gaps` | ギャップ解消モード（FINDINGS.mdを読み込み、調査をスキップ） |
| `--skip-verify` | プランチェッカーの検証ループをスキップ |
| `--prd <file>` | shape-hypothesisの代わりにPRDファイルをコンテキストとして使用 |
...
```bash
/hunt:plan 1                   # フェーズ1の調査＋計画＋検証
/hunt:plan 3 --skip-research   # 調査なしで計画（馴染みのあるドメイン）
/hunt:plan --auto              # 非対話型の計画
```
...
**生成物:** プランごとの `{phase}-{N}-SUMMARY.md`、gitコミット、フェーズ完了時に `{phase}-FINDINGS.md`

```bash
/hunt:run 1                # フェーズ1を実行
/hunt:run 1 --wave 2       # ウェーブ2のみを実行
```
...
**前提条件:** フェーズが実行済みであること
**生成物:** `{phase}-EVIDENCE_REVIEW.md`、問題が見つかった場合は修正プラン

```bash
/hunt:validate-findings 1                  # フェーズ1のEvidence Review
```
...
**動作:**
- プロジェクトなし → `/hunt:new-program` を提案
- フェーズにディスカッションが必要 → `/hunt:shape-hypothesis` を実行
- フェーズに計画が必要 → `/hunt:plan` を実行
- フェーズに実行が必要 → `/hunt:run` を実行
- フェーズに検証が必要 → `/hunt:validate-findings` を実行
- 全フェーズ完了 → `/thrunt:complete-milestone` を提案
...
```bash
/thrunt:session-report                 # セッション後のサマリーを生成
```
...
**前提条件:** フェーズが検証済み（`/hunt:validate-findings` が合格）、`gh` CLIがインストールされ認証済みであること
**生成物:** 計画アーティファクトからリッチな本文を持つGitHub PR、STATE.mdの更新

```bash
/hunt:publish 4                         # フェーズ4をシップ
/hunt:publish 4 --draft                 # ドラフトPRとしてシップ
```
...
**前提条件:** プロジェクトにフロントエンドコードがあること（単体で動作、THRUNTプロジェクト不要）
**生成物:** `{phase}-UI-REVIEW.md`、`.planning/ui-reviews/` 内のスクリーンショット

```bash
/thrunt:ui-review                      # 現在のフェーズを監査
/thrunt:ui-review 3                    # フェーズ3を監査
```
...
全フェーズを横断した未処理のEvidence Reviewおよび検証項目の監査。

**前提条件:** 少なくとも1つのフェーズがEvidence Reviewまたは検証付きで実行されていること
...
```bash
/thrunt:milestone-summary                # 現在のマイルストーンをサマリー
/thrunt:milestone-summary v1.0           # 特定のマイルストーンをサマリー
```
...
**生成物:** 更新された `MISSION.md`、新しい `HYPOTHESES.md`、新しい `HUNTMAP.md`

```bash
/hunt:new-program                  # 対話モード
/hunt:new-program "v2.0 Mobile"    # 名前付きマイルストーン
/hunt:new-program --reset-phase-numbers "v2.0 Mobile"  # マイルストーン番号を1からリスタート
```
...
```bash
/thrunt:add-phase                      # 対話型 — フェーズの説明を入力
```
...
```bash
/thrunt:insert-phase 3                 # フェーズ3と4の間に挿入 → 3.1を作成
```
...
```bash
/thrunt:remove-phase 7                 # フェーズ7を削除、8→7、9→8等に番号振り直し
```
...
```bash
/thrunt:list-phase-assumptions 2       # フェーズ2の前提を確認
```
...
```bash
/thrunt:plan-milestone-gaps             # 各監査ギャップに対してフェーズを作成
```

### `/hunt:shape-hypothesis`

詳細なエコシステム調査のみを実行します（単体機能 — 通常は `/hunt:plan` を使用してください）。
...
```bash
/hunt:shape-hypothesis 4               # フェーズ4のドメインを調査
```
...
```bash
/thrunt:validate-phase 2               # フェーズ2のテストカバレッジを監査
```
...
```bash
/thrunt:resume-work                    # コンテキストリセットまたは新しいセッション後に使用
```
...
```bash
/thrunt:pause-work                     # continue-here.mdを作成
```
...
```bash
/thrunt:manager                        # コマンドセンターダッシュボードを開く
```
...
```bash
/thrunt:quick                          # 基本的なクイックタスク
/thrunt:quick --discuss --research     # ディスカッション＋調査＋計画
/thrunt:quick --full                   # プランチェックと所見検証付き
/thrunt:quick --discuss --research --full  # すべてのオプションステージ
```
...
```bash
/thrunt:autonomous                     # 残りの全フェーズを実行
/thrunt:autonomous --from 3            # フェーズ3から開始
```

### `/thrunt:do`

フリーテキストを適切なTHRUNTコマンドにルーティングします。

```bash
/thrunt:do                             # その後、やりたいことを説明
```
...
```bash
/thrunt:add-tests 2                    # フェーズ2のテストを生成
```
...
```bash
/thrunt:stats                          # プロジェクトメトリクスダッシュボード
```
...
- `/thrunt:dev-preferences` コマンド — 任意のセッションでプリファレンスをロード
- `CLAUDE.md` プロファイルセクション — Claude Codeが自動検出

```bash
/thrunt:profile-user                   # セッションを分析してプロファイルを構築
/thrunt:profile-user --questionnaire   # 対話型アンケートのフォールバック
/thrunt:profile-user --refresh         # 新鮮な分析からの再生成
```
...
```bash
/thrunt:health                         # 整合性チェック
/thrunt:health --repair                # チェックして修復
```
...
```bash
/thrunt:forensics                              # 対話型 — 問題の入力を促す
/thrunt:forensics "Phase 3 execution stalled"  # 問題の説明付き
```
...
**前提条件:** アクティブなTHRUNTプロジェクト
**生成物:** `.planning/` 配下のワークストリームディレクトリ、ワークストリームごとの状態追跡

```bash
/thrunt:workstreams                    # すべてのワークストリームを一覧表示
/thrunt:workstreams create backend-api # 新しいワークストリームを作成
/thrunt:workstreams switch backend-api # アクティブなワークストリームを設定
/thrunt:workstreams status backend-api # 詳細ステータス
/thrunt:workstreams progress           # ワークストリーム横断の進捗概要
/thrunt:workstreams complete backend-api  # 完了したワークストリームをアーカイブ
/thrunt:workstreams resume backend-api    # ワークストリームでの作業を再開
```
...
```bash
/thrunt:set-profile budget             # budgetプロファイルに切り替え
/thrunt:set-profile quality            # qualityプロファイルに切り替え
```
...
```bash
/hunt:map-environment                   # コードベース全体を分析
/hunt:map-environment auth              # auth領域にフォーカス
```
...
変更履歴のプレビュー付きでTHRUNTをアップデートします。

```bash
/thrunt:update                         # アップデートを確認してインストール
```

### `/thrunt:reapply-patches`

THRUNTアップデート後にローカルの変更を復元します。

```bash
/thrunt:reapply-patches                # ローカルの変更をマージバック
```
...
**`/thrunt:quick` の代替ではありません** — 調査、複数ステップの計画、または検証が必要な場合は `/thrunt:quick` を使用してください。
...
**生成物:** `{phase}-REVIEWS.md` — `/hunt:plan --reviews` で利用可能
...
**目的:** レビュアーにはコード変更のみを表示し、THRUNT計画アーティファクトは含めません。

```bash
/thrunt:pr-branch                     # mainに対してフィルタリング
/thrunt:pr-branch develop             # developに対してフィルタリング
```
...
**999.x番号付け**により、バックログ項目はアクティブなフェーズシーケンスの外に保持されます。フェーズディレクトリは即座に作成されるため、`/hunt:shape-hypothesis` や `/hunt:plan` がそれらに対して動作します。
...
**利用先:** `/hunt:new-program`（シードをスキャンしてマッチするものを提示）
...
スレッドは、複数のセッションにまたがるが特定のフェーズに属さない作業のための軽量なクロスセッション知識ストアです。`/thrunt:pause-work` よりも軽量です。

```bash
/thrunt:thread                         # すべてのスレッドを一覧表示
/thrunt:thread fix-deploy-key-auth     # スレッドを再開
/thrunt:thread "Investigate TCP timeout in pasta service"  # 新規作成
```
