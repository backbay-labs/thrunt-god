...
| CRITICAL | <= 25% | 即座に停止し、状態を保存する（`/thrunt:pause-work`） |
...
```
ステータスラインフック (thrunt-statusline.js)
    | 書き込み
    v
/tmp/claude-ctx-{session_id}.json
    ^ 読み取り
    |
コンテキストモニター (thrunt-context-monitor.js, PostToolUse/AfterTool)
    | 注入
    v
...
THRUNT の `/thrunt:pause-work` コマンドは実行状態を保存します。WARNING メッセージはこのコマンドの使用を提案し、CRITICAL メッセージは即座の状態保存を指示します。

## セットアップ

両フックは `npx thrunt-god` のインストール時に自動的に登録されます:
