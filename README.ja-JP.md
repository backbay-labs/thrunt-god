...
だからTHRUNTを作りました。複雑さはシステムの中にあり、ワークフローの中にはありません。裏側では、コンテキストエンジニアリング、XMLプロンプトフォーマッティング、サブエージェントのオーケストレーション、状態管理が動いています。あなたが目にするのは、ただ動くいくつかのコマンドだけです。

このシステムは、Claudeが仕事をし、*かつ*検証するために必要なすべてを提供します。私はこのワークフローを信頼しています。ちゃんといい仕事をしてくれます。

これがTHRUNTです。エンタープライズごっこは一切なし。Claude Codeを使って一貫してクールなものを作るための、非常に効果的なシステムです。
...
THRUNTはそれを解決します。Claude Codeを信頼性の高いものにするコンテキストエンジニアリングレイヤーです。アイデアを説明し、システムに必要なすべてを抽出させ、Claude Codeに仕事をさせましょう。
...
> [!NOTE]
> Codexのインストールでは、カスタムプロンプトではなくスキル（`skills/thrunt-*/SKILL.md`）を使用します。

### 最新の状態を保つ

THRUNTは急速に進化しています。定期的にアップデートしてください：
...
```bash
# Claude Code
npx thrunt-god --claude --global   # ~/.claude/ にインストール
npx thrunt-god --claude --local    # ./.claude/ にインストール

# OpenCode（オープンソース、無料モデル）
npx thrunt-god --opencode --global # ~/.config/opencode/ にインストール

# Gemini CLI
npx thrunt-god --gemini --global   # ~/.gemini/ にインストール

# Codex（スキルファースト）
npx thrunt-god --codex --global    # ~/.codex/ にインストール
npx thrunt-god --codex --local     # ./.codex/ にインストール

# Copilot（GitHub Copilot CLI）
npx thrunt-god --copilot --global  # ~/.github/ にインストール
npx thrunt-god --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx thrunt-god --cursor --global      # ~/.cursor/ にインストール
npx thrunt-god --cursor --local       # ./.cursor/ にインストール

# Antigravity（Google、スキルファースト、Geminiベース）
npx thrunt-god --antigravity --global # ~/.gemini/antigravity/ にインストール
npx thrunt-god --antigravity --local  # ./.agent/ にインストール

# 全ランタイム
npx thrunt-god --all --global      # すべてのディレクトリにインストール
```
...
THRUNTは摩擦のない自動化のために設計されています。Claude Codeを以下のように実行してください：
...
> [!TIP]
> これがTHRUNTの意図された使い方です — `date` や `git commit` を50回も承認するために止まっていては目的が台無しです。
...
> **既存のコードがある場合は？** まず `/hunt:map-environment` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/hunt:new-program` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
**作成されるファイル：** `MISSION.md`、`HYPOTHESES.md`、`HUNTMAP.md`、`STATE.md`、`.planning/research/`
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/thrunt:settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
**作成されるファイル：** `{phase_num}-{N}-SUMMARY.md`、`{phase_num}-FINDINGS.md`
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/hunt:run` を再度実行するだけです。

**作成されるファイル：** `{phase_num}-EVIDENCE_REVIEW.md`、問題が見つかった場合は修正プラン
...
/hunt:publish 2                  # 検証済みの作業からPRを作成
...
/thrunt:complete-milestone
/hunt:new-program
```

またはTHRUNTに次のステップを自動判定させます：

```
/thrunt:next                    # 次のステップを自動検出して実行
```

**discuss → plan → execute → verify → publish** のループをマイルストーン完了まで繰り返します。

ディスカッション中のインプットを速くしたい場合は、`/hunt:shape-hypothesis <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/thrunt:complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/hunt:new-program` で次のバージョンを開始します — `new-program` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
クイックモードはTHRUNTの保証（アトミックコミット、状態トラッキング）をより速いパスで提供します：
...
| ファイル | 役割 |
|------|--------------|
| `MISSION.md` | プロジェクトビジョン、常に読み込まれる |
| `research/` | エコシステムの知識（スタック、機能、アーキテクチャ、落とし穴） |
| `HYPOTHESES.md` | フェーズとのトレーサビリティを持つスコープ済みv1/v2要件 |
| `HUNTMAP.md` | 進む方向、完了済みの作業 |
...
| コマンド | 説明 |
|---------|--------------|
| `/hunt:new-program [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/hunt:shape-hypothesis [N] [--auto] [--analyze]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加） |
| `/hunt:plan [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
| `/hunt:run <N>` | 全プランを並列ウェーブで実行し、完了時に検証 |
| `/hunt:validate-findings [N]` | 手動ユーザー受入テスト ¹ |
| `/hunt:publish [N] [--draft]` | 検証済みのフェーズ作業から自動生成された本文付きのPRを作成 |
| `/thrunt:next` | 次の論理的なワークフローステップに自動的に進む |
| `/thrunt:fast <text>` | インラインの軽微タスク — 計画を完全にスキップし即座に実行 |
| `/thrunt:audit-milestone` | マイルストーンが完了の定義を達成したか検証 |
| `/thrunt:complete-milestone` | マイルストーンをアーカイブし、リリースをタグ付け |
| `/hunt:new-program [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/thrunt:forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/thrunt:milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/thrunt:workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/thrunt:workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/thrunt:workstreams complete <name>` | ワークストリームを完了しマージ |

### マルチプロジェクトワークスペース

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:new-workspace` | リポジトリのコピー（worktreeまたはクローン）で隔離されたワークスペースを作成 |
| `/thrunt:list-workspaces` | すべてのTHRUNTワークスペースとそのステータスを表示 |
| `/thrunt:remove-workspace` | ワークスペースを削除しworktreeをクリーンアップ |

### UIデザイン

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/thrunt:ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |

### ナビゲーション

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:progress` | 今どこにいる？次は何？ |
| `/thrunt:next` | 状態を自動検出し次のステップを実行 |
| `/thrunt:help` | 全コマンドと使い方ガイドを表示 |
| `/thrunt:update` | チェンジログプレビュー付きでTHRUNTをアップデート |
| `/thrunt:join-discord` | THRUNT Discordコミュニティに参加 |
| `/thrunt:manager` | 複数フェーズ管理用のインタラクティブコマンドセンター |

### ブラウンフィールド

| コマンド | 説明 |
|---------|--------------|
| `/hunt:map-environment [area]` | new-program前に既存のコードベースを分析 |

### フェーズ管理

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:add-phase` | ロードマップにフェーズを追加 |
| `/thrunt:insert-phase [N]` | フェーズ間に緊急作業を挿入 |
| `/thrunt:remove-phase [N]` | 将来のフェーズを削除し番号を振り直し |
| `/thrunt:list-phase-assumptions [N]` | 計画前にClaudeの意図するアプローチを確認 |
| `/thrunt:plan-milestone-gaps` | 監査で見つかったギャップを埋めるフェーズを作成 |

### セッション

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:pause-work` | フェーズ途中で停止する際の引き継ぎを作成（HANDOFF.jsonを書き込み） |
| `/thrunt:resume-work` | 前回のセッションから復元 |
| `/thrunt:session-report` | 実行した作業と結果のセッションサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |

### コード品質

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:review` | 現在のフェーズまたはブランチのクロスAIピアレビュー |
| `/thrunt:pr-branch` | `.planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
| `/thrunt:audit-evidence` | 検証負債を監査 — Evidence Reviewが未実施のフェーズを検出 |

### バックログ & スレッド

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:plant-seed <idea>` | トリガー条件付きの将来志向のアイデアをキャプチャ — 適切なマイルストーンで浮上 |
| `/thrunt:add-backlog <desc>` | バックログのパーキングロットにアイデアを追加（999.xナンバリング、アクティブシーケンス外） |
| `/thrunt:review-backlog` | バックログ項目をレビューし、アクティブマイルストーンに昇格またはstaleエントリを削除 |
| `/thrunt:thread [name]` | 永続コンテキストスレッド — 複数セッションにまたがる作業用の軽量クロスセッション知識 |

### ユーティリティ

| コマンド | 説明 |
|---------|--------------|
| `/thrunt:settings` | モデルプロファイルとワークフローエージェントを設定 |
| `/thrunt:set-profile <profile>` | モデルプロファイルを切り替え（quality/balanced/budget/inherit） |
| `/thrunt:add-todo [desc]` | 後で取り組むアイデアをキャプチャ |
| `/thrunt:check-todos` | 保留中のtodoを一覧表示 |
| `/thrunt:debug [desc]` | 永続状態を持つ体系的デバッグ |
| `/thrunt:do <text>` | フリーフォームテキストを適切なTHRUNTコマンドに自動ルーティング |
| `/thrunt:note <text>` | ゼロフリクションのアイデアキャプチャ — ノートの追加、一覧、todoへの昇格 |
| `/thrunt:quick [--full] [--discuss] [--research]` | THRUNTの保証付きでアドホックタスクを実行（`--full` でプランチェックと検証を追加、`--discuss` で事前にコンテキストを収集、`--research` で計画前にアプローチを調査） |
| `/thrunt:health [--repair]` | `.planning/` ディレクトリの整合性を検証、`--repair` で自動修復 |
| `/thrunt:stats` | プロジェクト統計を表示 — フェーズ、プラン、要件、gitメトリクス |
| `/thrunt:profile-user [--questionnaire] [--refresh]` | セッション分析から開発者行動プロファイルを生成し、パーソナライズされた応答を提供 |
...
THRUNTはプロジェクト設定を `.planning/config.json` に保存します。`/hunt:new-program` 実行時に設定するか、後から `/thrunt:settings` で更新できます。完全な設定スキーマ、ワークフロートグル、gitブランチオプション、エージェントごとのモデル内訳については、[ユーザーガイド](docs/ja-JP/USER-GUIDE.md#configuration-reference)をご覧ください。
...
| `workflow.validator` | `true` | 実行後に必須項目が提供されたか確認 |
...
| `workflow.skip_discuss` | `false` | 自律モードでshape-hypothesisをスキップ |
| `workflow.text_mode` | `false` | リモートセッション用のテキスト専用モード（TUIメニューなし） |

これらのトグルには `/thrunt:settings` を使用するか、呼び出し時にオーバーライドできます：
- `/hunt:plan --skip-research`
- `/hunt:plan --skip-verify`
...
THRUNTが実行中にブランチをどう扱うかを制御します。

| 設定 | オプション | デフォルト | 説明 |
|---------|---------|---------|--------------|
| `git.branching_strategy` | `none`, `phase`, `milestone` | `none` | ブランチ作成戦略 |
| `git.phase_branch_template` | string | `thrunt/phase-{phase}-{slug}` | フェーズブランチのテンプレート |
| `git.milestone_branch_template` | string | `thrunt/{milestone}-{slug}` | マイルストーンブランチのテンプレート |

**戦略：**
- **`none`** — 現在のブランチにコミット（デフォルトのTHRUNT動作）
- **`phase`** — フェーズごとにブランチを作成し、フェーズ完了時にマージ
- **`milestone`** — マイルストーン全体で1つのブランチを作成し、完了時にマージ

マイルストーン完了時、THRUNTはスカッシュマージ（推奨）または履歴付きマージを提案します。
...
THRUNTはv1.27以降、多層防御セキュリティを備えています：
...
- **PreToolUseプロンプトガードフック** — `thrunt-prompt-guard` が `.planning/` への書き込みに埋め込まれたインジェクションベクトルをスキャン（アドバイザリー、ブロッキングではない）
...
> [!NOTE]
> THRUNTはLLMシステムプロンプトとなるマークダウンファイルを生成するため、計画成果物に流入するユーザー制御テキストは潜在的な間接プロンプトインジェクションベクトルとなります。これらの保護は、そのようなベクトルを複数のレイヤーで捕捉するように設計されています。

### 機密ファイルの保護

THRUNTのコードベースマッピングおよび分析コマンドは、プロジェクトを理解するためにファイルを読み取ります。**シークレットを含むファイルを保護する**には、Claude Codeの拒否リストに追加してください：
...
> [!IMPORTANT]
> THRUNTにはシークレットのコミットに対する組み込み保護がありますが、多層防御がベストプラクティスです。防御の第一線として、機密ファイルへの読み取りアクセスを拒否してください。
...
- `~/.claude/commands/thrunt/`（グローバル）または `./.claude/commands/thrunt/`（ローカル）にファイルが存在するか確認してください
- Codexの場合、`~/.codex/skills/thrunt-*/SKILL.md`（グローバル）または `./.codex/skills/thrunt-*/SKILL.md`（ローカル）にスキルが存在するか確認してください

**コマンドが期待通りに動作しない？**
- `/thrunt:help` を実行してインストールを確認してください
- `npx thrunt-god` を再実行して再インストールしてください
...
npx thrunt-god --antigravity --local --uninstall
```

これにより、他の設定を保持しながら、すべてのTHRUNTコマンド、エージェント、フック、設定が削除されます。

---

## コミュニティポート

OpenCode、Gemini CLI、Codexは `npx thrunt-god` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [thrunt-opencode](https://github.com/rokicool/thrunt-opencode) | OpenCode | オリジナルのOpenCode対応版 |
| thrunt-gemini（アーカイブ済み） | Gemini CLI | uberfuzzyによるオリジナルのGemini対応版 |
...
**Claude Codeは強力です。THRUNTはそれを信頼性の高いものにします。**
