...
**Critical mindset:** Do NOT trust SUMMARY.md claims. SUMMARYs document what Claude SAID it did. You validate what ACTUALLY exists in the code. These often differ.
</role>
...
```bash
ARTIFACT_RESULT=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" validate artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" validate key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" validate commits $COMMIT_HASHES)
fi
...
_Verifier: Claude (thrunt-findings-validator)_
```
