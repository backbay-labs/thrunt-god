...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" validate plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" validate plan-structure "$PLAN_PATH")
```
