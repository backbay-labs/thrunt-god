/**
 * Hunt history -- pure functional hunt history section management.
 *
 * Manages the ## Hunt History section for entity notes:
 * - Typed hunt roles (target, indicator, infrastructure, false_positive)
 * - Build hunt history markdown sections from structured entries
 * - Insert/replace ## Hunt History sections in entity note content
 *
...
export type HuntRole = 'target' | 'indicator' | 'infrastructure' | 'false_positive';
...
const PLACEHOLDER = '_No hunt references found._';
...
/**
 * Build a markdown ## Hunt History section from structured entries.
 *
 * Format: `- **{huntId}** ({date}) -- role: {role}, outcome: {outcome}`
 *
 * Returns placeholder text when entries is empty.
 */
export function buildHuntHistorySection(entries: HuntHistoryEntry[]): string {
  if (entries.length === 0) {
    return `${HUNT_HISTORY_HEADING}\n\n${PLACEHOLDER}\n`;
  }

  const lines = entries.map(
    (e) => `- **${e.huntId}** (${e.date}) -- role: ${e.role}, outcome: ${e.outcome}`,
  );
...
/**
 * Insert or replace the ## Hunt History section in entity note content.
 *
 * Section placement priority:
 * 1. Replace existing ## Hunt History section
 * 2. Insert after ## Verdict History (before next ## heading)
 ...
 */
export function appendHuntHistorySection(
  content: string,
  entries: HuntHistoryEntry[],
): string {
  const sectionContent = buildHuntHistorySection(entries);
  const sectionLines = sectionContent.split('\n');
  // Remove trailing empty string from split (buildHuntHistorySection ends with \n)
  ...
  // Case 2: Insert after ## Verdict History
  const verdictIdx = lines.findIndex((l) => l.trim() === '## Verdict History');
  if (verdictIdx !== -1) {
    const endIdx = findSectionEnd(lines, verdictIdx);
