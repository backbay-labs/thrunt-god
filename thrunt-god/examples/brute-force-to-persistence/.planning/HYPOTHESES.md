# Hypotheses: Meridian Identity Abuse Program

## Active Hypotheses

### HYP-01: Program intake artifacts are complete and parseable

- **Signal:** The example workspace opens on `.planning/MISSION.md` and exposes a completed program shell
- **Assertion:** The Meridian program root contains the singleton artifacts the extension expects and they parse into a coherent program-level investigation context
- **Priority:** High
...
- **Signal:** The program includes one closed child case under `cases/brute-force-to-persistence`
...
- **Priority:** High
- **Scope:** `cases/brute-force-to-persistence/` plus published findings path
...
- **Signal:** Program `STATE.md` reports 9 unique ATT&CK techniques for the Meridian case
...
- The program layer is intentionally static and read-only; it exists to demonstrate dashboard and lineage views over a completed case.
- The underlying compromise investigation remains in `cases/brute-force-to-persistence/`.
