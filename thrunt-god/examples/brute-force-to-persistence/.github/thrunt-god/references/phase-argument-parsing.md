...
```bash
PHASE_INFO=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" find-phase "${PHASE}" --raw)
```
