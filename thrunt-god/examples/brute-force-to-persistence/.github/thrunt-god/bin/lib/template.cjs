...
const { normalizePhaseName, findPhaseInternal, generateSlugInternal, normalizeMd, toPosixPath, getMissionDocInfo, getHuntmapDocInfo, planningPaths, output, error } = require('./core.cjs');
...
function cmdTemplateFill(cwd, templateType, options, raw) {
  if (!templateType) { error('template type required: summary, plan, evidence-review, or findings'); }
  ...
  const contextDocs = [
    `@${toPosixPath(path.relative(cwd, projectDoc.path))}`,
    `@${toPosixPath(path.relative(cwd, roadmapDoc.path))}`,
    '@.planning/STATE.md',
  ];
  if (projectDoc.source === 'MISSION.md') {
    if (fs.existsSync(paths.hypotheses)) {
      contextDocs.push(`@${toPosixPath(path.relative(cwd, paths.hypotheses))}`);
    }
    if (fs.existsSync(paths.successCriteria)) {
      contextDocs.push(`@${toPosixPath(path.relative(cwd, paths.successCriteria))}`);
    }
  }
  ...
  switch (templateType) {
    ...
    case 'evidence-review': {
      ...
      body = [
        `# Phase ${options.phase}: ${phaseName} — Evidence Review`,
        '',
      ...
      fileName = `${padded}-EVIDENCE_REVIEW.md`;
      break;
    }
    case 'findings': {
      frontmatter = {
      ...
      body = [
        `# Phase ${options.phase}: ${phaseName} — Findings`,
        '',
        '## Executive Summary',
        '',
      ...
      fileName = `${padded}-FINDINGS.md`;
      break;
    }
    default:
      error(`Unknown template type: ${templateType}. Available: summary, plan, evidence-review, findings`);
      return;
  }
