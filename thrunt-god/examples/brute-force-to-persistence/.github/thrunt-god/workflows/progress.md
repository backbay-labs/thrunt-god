...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init progress)
...
```bash
DISCUSS_MODE=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
- `STATE=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node ".github/thrunt-god/bin/thrunt-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" audit-evidence --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
- `/thrunt-list-phase-assumptions {phase}` — see the agent's assumptions
...
- `/thrunt-list-phase-assumptions {phase} ${THRUNT_WS}` — see the agent's assumptions
...
```bash
NEXT_PHASE_SECTION=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase "$((Z+1))" 2>/dev/null)
