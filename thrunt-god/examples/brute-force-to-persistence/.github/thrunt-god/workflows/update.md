<purpose>
Check for THRUNT updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether THRUNT is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/thrunt-update` targets the runtime that invoked it.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/thrunt-god/VERSION" ] || [ -f "./$dir/thrunt-god/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/thrunt-god/VERSION"
    LOCAL_MARKER_FILE="./$dir/thrunt-god/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/thrunt-god/VERSION" ] || [ -f "$HOME/$dir/thrunt-god/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/thrunt-god/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/thrunt-god/workflows/update.md"
...
```bash
npm view thrunt-god version 2>/dev/null
```
...
To update manually: `npx thrunt-god --global`
```
...
⚠️  **Note:** The installer performs a clean install of THRUNT folders:
- `commands/thrunt/` will be wiped and replaced
- `thrunt-god/` will be wiped and replaced
- `agents/thrunt-*` files will be replaced
...
- Custom commands not in `commands/thrunt/` ✓
- Custom agents not prefixed with `thrunt-` ✓
- Custom hooks ✓
- Your copilot-instructions.md files ✓

If you've modified any THRUNT files directly, they'll be automatically backed up to `thrunt-local-patches/` and can be reapplied with `/thrunt-reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y thrunt-god@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y thrunt-god@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y thrunt-god@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/thrunt-update-check.json"
  rm -f "$HOME/$dir/cache/thrunt-update-check.json"
done
```

The SessionStart hook (`thrunt-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  THRUNT Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/thrunt-build/thrunt-god/blob/main/CHANGELOG.md)
```
</step>
...
Check for thrunt-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /thrunt-reapply-patches to merge your modifications into the new version.
```
