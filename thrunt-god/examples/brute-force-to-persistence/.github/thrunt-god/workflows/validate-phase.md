...
AGENT_SKILLS_AUDITOR=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-false-positive-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-false-positive-auditor --raw)
NYQUIST_CFG=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read .github/agents/thrunt-false-positive-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `.github/thrunt-god/templates/VALIDATION.md`
...
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
