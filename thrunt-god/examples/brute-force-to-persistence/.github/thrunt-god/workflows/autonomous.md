...
Drive all remaining milestone phases autonomously. For each incomplete phase: discuss → plan → execute using Skill() flat invocations. Pauses only for explicit user decisions (grey area acceptance, blockers, validation requests). Re-reads HUNTMAP.md after each phase to catch dynamically inserted phases.
...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `huntmap_exists`, `state_exists`, `commit_docs`.

**If `huntmap_exists` is false:** Error — "No HUNTMAP.md found. Run `/hunt-new-program` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/hunt-new-program` first."
...
```bash
HUNTMAP=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap analyze)
```

Parse the JSON `phases` array.

**Filter to incomplete phases:** Keep only phases where `disk_status !== "complete"` OR `huntmap_complete === false`.
...
```bash
DETAIL=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

Where N = current phase number (from the HUNTMAP, e.g., 6), T = total milestone phases (from `phase_count` parsed in initialize step, e.g., 8), P = percentage of all milestone phases completed so far. Calculate P as: (number of phases with `disk_status` "complete" from the latest `huntmap analyze` / T × 100). Use █ for filled and ░ for empty segments in the progress bar (8 characters wide).
...
```bash
PHASE_STATE=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```

**If SKIP_DISCUSS is `true`:** Skip discuss entirely — the HUNTMAP phase description is the spec. Display:

```
Phase ${PHASE_NUM}: Discuss skipped (workflow.skip_discuss=true) — using HUNTMAP phase goal as spec.
```

Write a minimal CONTEXT.md so downstream hunt-plan has valid input. Get phase details:
...
{goal from HUNTMAP phase description}
...
All implementation choices are at the agent's discretion — discuss phase was skipped per user setting. Use HUNTMAP phase goal, success criteria, and codebase conventions to guide decisions.
...
Codebase context will be gathered during hunt-plan research.
...
No specific requirements — discuss phase skipped. Refer to HUNTMAP phase description and success criteria.
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="hunt:plan", args="${PHASE_NUM}")
```
...
```
Skill(skill="hunt:run", args="${PHASE_NUM} --no-transition")
```

**3d. Post-Execution Routing**

After hunt-run returns, read the validation result:

```bash
VERIFY_STATUS=$(grep "^status:" "${PHASE_DIR}"/*-FINDINGS.md 2>/dev/null | head -1 | cut -d: -f2 | tr -d ' ')
```
...
**If VERIFY_STATUS is empty** (no FINDINGS.md or no status field):

Go to handle_blocker: "Execute phase ${PHASE_NUM} did not produce validation results."

**If `passed`:**

Display:
```
Phase ${PHASE_NUM} ✅ ${PHASE_NAME} — Validation passed
```
...
Read the human_verification section from FINDINGS.md to get the count and items requiring manual testing.

Display the items, then ask user via AskUserQuestion:
- **question:** "Phase ${PHASE_NUM} has items needing manual validation. Validate now or continue to next phase?"
- **options:** "Validate now" / "Continue without validation"

On **"Validate now"**: Present the specific items from FINDINGS.md's human_verification section. After user reviews, ask:
...
Read gap summary from FINDINGS.md (score and missing items). Display:
```
...
```
Skill(skill="hunt:plan", args="${PHASE_NUM} --gaps")
```
...
Run smart discuss for the current phase. Proposes grey area answers in batch tables — the user accepts or overrides per area. Produces identical CONTEXT.md output to regular shape-hypothesis.

> **Note:** Smart discuss is an autonomous-optimized variant of the `hunt:shape-hypothesis` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `shape-hypothesis` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
cat .planning/MISSION.md 2>/dev/null || true
cat .planning/HYPOTHESES.md 2>/dev/null || true
cat .planning/STATE.md 2>/dev/null || true
```

Extract from these:
- **MISSION.md** — Vision, principles, non-negotiables, user preferences
- **HYPOTHESES.md** — Acceptance criteria, constraints, must-haves vs nice-to-haves
...
<prior_decisions>
## Program-Level
- [Key principle or constraint from MISSION.md]
- [Hypothesis affecting this phase from HYPOTHESES.md]
...
- `<domain>`: Phase boundary from HUNTMAP goal
...
- **Pre-select a recommended answer** based on: prior decisions (consistency), codebase patterns (reuse), domain conventions (standard approaches), HUNTMAP success criteria
...
Use **exactly** this structure (identical to shape-hypothesis output):
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
After each phase completes, re-read HUNTMAP.md to catch phases inserted mid-execution (decimal phases like 5.1):
...
- Keep phases where `disk_status !== "complete"` OR `huntmap_complete === false`
...
```
Skill(skill="thrunt:audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /thrunt-audit-milestone to review, then /thrunt-complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /thrunt-audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="thrunt:complete-milestone", args="${milestone_version}")
```

After complete-milestone returns, verify it produced output:

```bash
ls .planning/milestones/v${milestone_version}-HUNTMAP.md 2>/dev/null || true
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /thrunt-autonomous --from {next_phase}
```
...
- [ ] Post-execution validation reads FINDINGS.md and routes on status
- [ ] Passed validation → automatic continue to next phase
- [ ] Human-needed validation → user prompted to validate or skip
- [ ] Gaps-found → user offered gap closure, continue, or stop
- [ ] Gap closure limited to 1 retry (prevents infinite loops)
- [ ] Plan-phase and hunt-run failures route to handle_blocker
- [ ] HUNTMAP.md re-read after each phase (catches inserted phases)
...
- [ ] Audit invoked via Skill(skill="thrunt:audit-milestone")
...
- [ ] Smart discuss documents relationship to shape-hypothesis with CTRL-03 note
</success_criteria>
