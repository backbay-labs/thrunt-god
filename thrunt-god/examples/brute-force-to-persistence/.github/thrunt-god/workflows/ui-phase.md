<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates thrunt-ui-researcher and thrunt-ui-checker with a revision loop. Inserts between shape-hypothesis and hunt-plan in the lifecycle.
...
- thrunt-ui-researcher — Researches UI/UX approaches
- thrunt-ui-checker — Reviews UI implementation quality
</available_agent_types>
...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init plan "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_UI=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-ui-checker 2>/dev/null)
```

Parse JSON for: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `has_context`, `has_research`, `commit_docs`.

**File paths:** `state_path`, `huntmap_path`, `hypotheses_path`, `context_path`, `research_path`.

Resolve UI agent models:

```bash
UI_RESEARCHER_MODEL=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-ui-researcher --raw)
UI_CHECKER_MODEL=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /thrunt-settings.
```
Exit workflow.

**If `planning_exists` is false:** Error — run `/hunt-new-program` first.
...
```bash
PHASE_INFO=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase "${PHASE}")
```
...
Recommended: run /hunt-shape-hypothesis {N} first to capture design preferences.
...
## 5. Spawn thrunt-ui-researcher

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT ► UI DESIGN CONTRACT — PHASE {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read .github/agents/thrunt-ui-researcher.md for instructions.
...
- {huntmap_path} (Huntmap)
- {hypotheses_path} (Hypotheses)
- {context_path} (USER DECISIONS from /hunt-shape-hypothesis)
...
Template: .github/thrunt-god/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="thrunt-ui-researcher",
...
## 7. Spawn thrunt-ui-checker

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT ► VERIFYING UI-SPEC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read .github/agents/thrunt-ui-checker.md for instructions.
...
Task(
  ...
  subagent_type="thrunt-ui-checker",
...
- Re-spawn thrunt-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /thrunt-ui-phase
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] thrunt-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] thrunt-ui-checker spawned with UI-SPEC.md
