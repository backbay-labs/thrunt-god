...
- inspect the pack first with `node ".github/thrunt-god/bin/thrunt-tools.cjs" pack bootstrap <id> ...`
...
- use `node ".github/thrunt-god/bin/thrunt-tools.cjs" config-new-program '{"mode":"interactive"}'` for normal bootstrap
- use `node ".github/thrunt-god/bin/thrunt-tools.cjs" config-new-program '{"mode":"yolo"}'` when `--auto` is present
...
- if you need to add or update connector profiles, use `node ".github/thrunt-god/bin/thrunt-tools.cjs" config-set connector_profiles.<connector>.<profile> '<json>'` instead of editing JSON by hand
...
Do not generate or update `copilot-instructions.md` during hunt bootstrap. The bootstrap must
