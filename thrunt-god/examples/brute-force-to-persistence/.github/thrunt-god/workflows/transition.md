...
There is no `/thrunt-transition` command. This workflow is invoked automatically by
`hunt-run` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/thrunt-transition`.

**Valid user commands for phase progression:**
- `/hunt-shape-hypothesis {N}` — discuss a phase before planning
- `/hunt-plan {N}` — plan a phase
- `/hunt-run {N}` — execute a phase
- `/thrunt-progress` — see huntmap progress
...
Mark current phase complete and advance to next. This is the natural point where progress tracking and MISSION.md evolution happen.
...
**Check for validation debt in this phase:**

```bash
# Count outstanding items in current phase
OUTSTANDING=""
for f in .planning/phases/XX-current/*-EVIDENCE_REVIEW.md .planning/phases/XX-current/*-FINDINGS.md; do
  [ -f "$f" ] || continue
...
```
Outstanding validation items in this phase:
{list filenames}

These will carry forward as debt. Review: `/thrunt-audit-evidence`
```
...
<step name="update_huntmap_and_state">

**Delegate HUNTMAP.md and STATE.md updates to thrunt-tools:**

```bash
TRANSITION=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" phase complete "${current_phase}")
```
...
Evolve MISSION.md to reflect learnings from completed phase.
...
1. **Hypotheses validated?**
   - Any Active requirements published in this phase?
   - Move to Validated with phase reference: `- ✓ [Hypothesis] — Phase X`

2. **Hypotheses invalidated?**
   - Any Active requirements discovered to be unnecessary or wrong?
   - Move to Out of Scope with reason: `- [Hypothesis] — [why invalidated]`

3. **Hypotheses emerged?**
...
After (Phase 2 published JWT auth, discovered rate limiting needed):
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `thrunt-tools phase complete` in the update_huntmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" progress bar --raw)
```
...
Update Mission Reference section in STATE.md.
...
- Full log lives in MISSION.md Key Decisions table
...
- [ ] Recent decisions noted (full log in MISSION.md)
...
**Use the transition result from `thrunt-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `THRUNT_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$THRUNT_WORKSTREAM" ]; then
  WS_LIST=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$THRUNT_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/hunt-plan [X+1] --auto ${THRUNT_WS}")
...
Exit skill and invoke SlashCommand("/hunt-shape-hypothesis [X+1] --auto ${THRUNT_WS}")
...
**Phase [X+1]: [Name]** — [Goal from HUNTMAP.md]

`/hunt-shape-hypothesis [X+1] ${THRUNT_WS}` — gather context and clarify approach
...
**Also available:**
- `/hunt-plan [X+1] ${THRUNT_WS}` — skip discussion, plan directly
- `/hunt-shape-hypothesis [X+1] ${THRUNT_WS}` — investigate unknowns
...
**Also available:**
- `/hunt-shape-hypothesis [X+1] ${THRUNT_WS}` — revisit context
- `/hunt-shape-hypothesis [X+1] ${THRUNT_WS}` — investigate unknowns
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/thrunt-workstreams complete {current_ws_name} ${THRUNT_WS}`

See overall milestone progress:

`/thrunt-workstreams progress ${THRUNT_WS}`
...
Do NOT suggest `/thrunt-complete-milestone` or `/hunt-new-program`.
...
Exit skill and invoke SlashCommand("/thrunt-complete-milestone {version} ${THRUNT_WS}")
...
`/thrunt-complete-milestone {version} ${THRUNT_WS}`
...
- Update HUNTMAP: "2/3 plans complete" (not "3/3")
...
- [ ] HUNTMAP.md updated with completion status and plan count
- [ ] MISSION.md evolved (requirements, decisions, description if needed)
