...
Key references:
- @.github/thrunt-god/references/ui-brand.md (display patterns)
- @.github/thrunt-god/agents/thrunt-analyst-profiler.md (profiler agent definition)
- @.github/thrunt-god/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".github/thrunt-god/USER-PROFILE.md"
...
```bash
cp ".github/thrunt-god/USER-PROFILE.md" ".github/thrunt-god/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .github/thrunt-god/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .github/thrunt-god/bin/thrunt-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .github/thrunt-god/bin/thrunt-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.github/thrunt-god/references/user-profiling.md`
...
Reference: @.github/thrunt-god/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .github/thrunt-god/bin/thrunt-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .github/thrunt-god/bin/thrunt-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .github/thrunt-god/bin/thrunt-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .github/thrunt-god/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .github/thrunt-god/USER-PROFILE.md" and jump to step 10.
...
```bash
node .github/thrunt-god/bin/thrunt-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /thrunt-dev-preferences at .github/commands/thrunt/dev-preferences.md"

**For copilot-instructions.md profile section (if selected):**

```bash
node .github/thrunt-god/bin/thrunt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .github/thrunt-god/bin/thrunt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH=".github/thrunt-god/USER-PROFILE.backup.md"
```
...
Your profile:    .github/thrunt-god/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /thrunt-dev-preferences   .github/commands/thrunt/dev-preferences.md
