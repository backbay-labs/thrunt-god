...
@.github/thrunt-god/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init run "${PHASE}")
...
# config settings can be fetched via thrunt-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="thrunt-telemetry-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn thrunt-telemetry-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" phases list --type summaries --raw
...
See `.github/thrunt-god/references/tdd.md` for structure.
</tdd_plan_execution>
...
**If running as a parallel executor agent (spawned by hunt-run):**
...
```bash
node .github/thrunt-god/bin/thrunt-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See .github/thrunt-god/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.github/thrunt-god/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.github/thrunt-god/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.github/thrunt-god/templates/summary.md`.

**Frontmatter:** phase, plan, subsystem, tags | requires/provides/affects | tech-stack.added/patterns | key-files.created/modified | key-decisions | hypotheses-completed (**MUST** copy `requirements` array from PLAN.md frontmatter verbatim) | duration ($DURATION), completed ($PLAN_END_TIME date).
...
Update STATE.md using thrunt-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ".github/thrunt-god/bin/thrunt-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node ".github/thrunt-god/bin/thrunt-tools.cjs" state update-progress

# Record execution metrics
node ".github/thrunt-god/bin/thrunt-tools.cjs" state record-metric \
...
node ".github/thrunt-god/bin/thrunt-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node ".github/thrunt-god/bin/thrunt-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using thrunt-tools:

```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" state record-session \
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" huntmap update-plan-progress "${PHASE}"
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" hypotheses mark-complete ${REQ_IDS}
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/HUNTMAP.md .planning/HYPOTHESES.md
```
</step>
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/hunt-run {phase}` + `/hunt-validate-findings`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/hunt-plan {Z+1}` + `/hunt-validate-findings {Z}` + `/hunt-shape-hypothesis {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/thrunt-complete-milestone` + `/hunt-validate-findings` + `/thrunt-add-phase` |
