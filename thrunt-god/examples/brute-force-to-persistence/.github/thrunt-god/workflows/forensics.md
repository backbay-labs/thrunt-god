...
Post-mortem investigation for failed or stuck THRUNT workflows. Analyzes git history,
...
> 'hunt-run failed silently', 'costs seem unusually high'."
...
- `.planning/HUNTMAP.md` — phase list with status
...
- `{padded}-FINDINGS.md` (quality verification)
...
**Signal:** Phase appears complete (has commits, is past in huntmap) but lacks expected artifacts.
...
- FINDINGS.md missing → quality check was skipped
...
3. {Recovery command if applicable — e.g., `/thrunt-resume-work`, `/hunt-run N`}

---

*Report generated by `/thrunt-forensics`. All paths redacted for portability.*
```
