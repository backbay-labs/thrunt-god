...
3. If neither found, check if `.planning/HUNTMAP.md` exists (project may be mid-milestone)
4. If nothing found: error "No milestone found. Run /hunt-new-program or /hunt-new-program first."
...
**Archived milestone** (`.planning/milestones/v{VERSION}-HUNTMAP.md` exists):
```
HUNTMAP_PATH=".planning/milestones/v${VERSION}-HUNTMAP.md"
HYPOTHESES_PATH=".planning/milestones/v${VERSION}-HYPOTHESES.md"
AUDIT_PATH=".planning/milestones/v${VERSION}-MILESTONE-AUDIT.md"
```

**Current/in-progress milestone** (no archive yet):
```
HUNTMAP_PATH=".planning/HUNTMAP.md"
HYPOTHESES_PATH=".planning/HYPOTHESES.md"
...
**Always available:**
```
PROJECT_PATH=".planning/MISSION.md"
...
```bash
thrunt-tools.cjs init progress
```
...
- Read `{phase_dir}/{padded}-FINDINGS.md` if it exists — extract status, gaps, deferred items
...
**If no phase directories exist** (empty milestone or pre-build state): skip to Step 5 and generate a minimal summary noting "No phases have been executed yet." Do not error — the summary should still capture MISSION.md and HUNTMAP.md content.
...
{From MISSION.md: "What This Is", core value proposition, target users}
...
{From MISSION.md: tech stack if documented}
...
## 4. Hypothesis Coverage

{From HYPOTHESES.md: list each requirement with status}
- ✅ {Hypothesis met}
- ⚠️ {Hypothesis partially met — note gap}
- ❌ {Hypothesis not met — note reason}
...
{From FINDINGS.md files: gaps found, anti-patterns noted}
...
- **Run the project:** {from MISSION.md or SUMMARY.md}
- **Key directories:** {from codebase structure}
- **Tests:** {test command from MISSION.md or copilot-instructions.md}
...
```bash
thrunt-tools.cjs commit "docs(v${VERSION}): generate milestone summary for onboarding" \
...
- Answer from the artifacts already loaded (CONTEXT.md, SUMMARY.md, FINDINGS.md, etc.)
- Reference specific files and decisions
- Stay grounded in what was actually built (not speculation)

If the user is done:
- Suggest next steps: `/hunt-new-program`, `/thrunt-progress`, or sharing the summary with the team

## Step 9: Update STATE.md

```bash
thrunt-tools.cjs state record-session \
