...
AGENT_SKILLS_UI_REVIEWER=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-ui-reviewer 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-ui-auditor --raw)
```
...
```markdown
Read .github/agents/thrunt-ui-auditor.md for instructions.
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
