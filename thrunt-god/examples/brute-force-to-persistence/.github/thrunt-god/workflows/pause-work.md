...
```bash
timestamp=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
