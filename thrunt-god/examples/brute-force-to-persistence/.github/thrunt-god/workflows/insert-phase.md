...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" phase insert "${after_phase}" "${description}")
```
