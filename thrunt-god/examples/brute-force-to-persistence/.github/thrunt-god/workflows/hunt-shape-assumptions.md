...
- thrunt-scope-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **thrunt-query-writer** — Reads CONTEXT.md to know WHAT to research
2. **thrunt-hunt-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
The phase boundary comes from HUNTMAP.md and is FIXED. Discussion clarifies HOW to implement
what's scoped, never WHETHER to add new capabilities.

When user suggests scope creep:
"[Feature X] would be a new capability — that's its own phase.
Want me to note it for the huntmap backlog? For now, let's focus on [phase domain]."
...
```bash
INIT=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ANALYZER=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-scope-analyzer 2>/dev/null)
```

Parse JSON for: `commit_docs`, `phase_found`, `phase_dir`, `phase_number`, `phase_name`,
`phase_slug`, `padded_phase`, `has_research`, `has_context`, `has_plans`, `has_findings`,
`plan_count`, `huntmap_exists`, `planning_exists`.

**If `phase_found` is false:**
```
Phase [X] not found in huntmap.

Use /thrunt-progress to see available phases.
```
Exit workflow.
...
- After completion, auto-advance to hunt-plan
</step>
...
- **HYPOTHESES.md** — Acceptance criteria, constraints
...
```bash
TODO_MATCHES=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
Spawn a `thrunt-scope-analyzer` agent to deeply analyze the codebase for this phase. This
...
- Source 1: Copy `Canonical refs:` from HUNTMAP.md for this phase, expand to full paths
- Source 2: Check HYPOTHESES.md and MISSION.md for specs/ADRs referenced
...
{Domain boundary from HUNTMAP.md — clear statement of scope anchor}
</domain>
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Phase ${PHASE}: {phase_name}** — {Goal from HUNTMAP.md}
...
**Also available:**
- `/hunt-plan ${PHASE} --skip-research` — plan without research
- `/thrunt-ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
2. Sync chain flag:
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node ".github/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```

**If `--auto` flag present AND `AUTO_CHAIN` is not true:**
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active true
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT ► AUTO-ADVANCING TO PLAN
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Context captured (assumptions mode). Launching hunt-plan...
```

Launch: `Skill(skill="hunt:plan", args="${PHASE} --auto")`

Handle return: PHASE COMPLETE / PLANNING COMPLETE / INCONCLUSIVE / GAPS FOUND
(identical handling to hunt-shape-hypothesis.md auto_advance step)
