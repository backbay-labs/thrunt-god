...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/thrunt-note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/thrunt-note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a THRUNT project. Run `/hunt-new-program` to initialize one."
...
status: pending
priority: P2
source: "promoted from /thrunt-note"
created: {YYYY-MM-DD}
theme: general
---
...
<edge_cases>
1. **"list" as note text**: `/thrunt-note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `.github/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/hunt-new-program`
