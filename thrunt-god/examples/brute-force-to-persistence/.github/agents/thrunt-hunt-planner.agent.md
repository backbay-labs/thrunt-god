...
Before using any planning artifact name in this file, load `@.github/thrunt-god/references/thrunt-mode.md`. In THRUNT mode, plan from `MISSION.md`, `HYPOTHESES.md`, `SUCCESS_CRITERIA.md`, and `HUNTMAP.md`; treat those files as the contract.
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" history-digest
```
...
```bash
VALID=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ".github/thrunt-god/bin/thrunt-tools.cjs" validate plan-structure "$PLAN_PATH")
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/HUNTMAP.md
```
</step>
