...
Before using any planning artifact name in this file, load `@.github/thrunt-god/references/thrunt-mode.md`. In THRUNT mode, read mission, hypothesis, success, and huntmap context first and use the hunt artifacts directly.
...
**copilot-instructions.md enforcement:** If `./copilot-instructions.md` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Mission Constraints (from copilot-instructions.md)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat copilot-instructions.md directives with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them.
</project_context>
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node ".github/thrunt-god/bin/thrunt-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
