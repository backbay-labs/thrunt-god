<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/thrunt:resume-work`; the markdown provides human-readable context.
</purpose>
...
```bash
timestamp=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
