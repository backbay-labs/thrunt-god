...
- thrunt-false-positive-auditor — Validates verification coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-false-positive-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-false-positive-auditor --raw)
NYQUIST_CFG=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /thrunt:settings."

Display banner: `THRUNT > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /hunt:run {N} ${THRUNT_WS} first."
...
### 2b. Build Hypothesis-to-Task Map
...
## 5. Spawn thrunt-false-positive-auditor

```
Task(
  prompt="Read ~/.claude/agents/thrunt-false-positive-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>" +
    "${AGENT_SKILLS_AUDITOR}",
  subagent_type="thrunt-false-positive-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/thrunt-god/templates/VALIDATION.md`
...
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
THRUNT > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /thrunt:audit-milestone ${THRUNT_WS}
```

**Partial:**
```
THRUNT > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /thrunt:validate-phase {N} ${THRUNT_WS}
```
