...
```bash
TRANSITION=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" progress bar --raw)
```
...
if [ -n "$THRUNT_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active false
```
