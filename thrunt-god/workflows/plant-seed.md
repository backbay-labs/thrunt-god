...
Seeds auto-surface during /hunt:new-program when trigger conditions match the
...
This seed should be presented during `/hunt:new-program` when the milestone
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /hunt:new-program
