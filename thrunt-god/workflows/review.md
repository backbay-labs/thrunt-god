...
Each CLI gets the same prompt (MISSION.md context, phase plans, requirements) and
...
Then read:
1. `.planning/MISSION.md` (first 80 lines — project context)
2. Phase section from `.planning/HUNTMAP.md`
3. All `*-PLAN.md` files in the phase directory
4. `*-CONTEXT.md` if present (user decisions)
5. `*-RESEARCH.md` if present (domain research)
6. `.planning/HYPOTHESES.md` (requirements this phase addresses)
</step>
...
## Mission Context
{first 80 lines of MISSION.md}

## Phase {N}: {phase name}
### Huntmap Section
{huntmap phase section}
...
Write to a temp file: `/tmp/thrunt-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/thrunt-review-prompt-{phase}.md)" 2>/dev/null > /tmp/thrunt-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/thrunt-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/thrunt-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/thrunt-review-prompt-{phase}.md)" 2>/dev/null > /tmp/thrunt-review-codex-{phase}.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT ► CROSS-AI REVIEW — Phase {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Commit:
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/hunt:plan --reviews)
</success_criteria>
