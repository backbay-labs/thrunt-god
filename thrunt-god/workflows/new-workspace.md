...
```bash
INIT=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /thrunt:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /thrunt:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /hunt:new-program    # Initialize THRUNT in the workspace
```
...
**Offer to initialize THRUNT (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize THRUNT"
- question: "Would you like to initialize a THRUNT project in the new workspace?"
- options:
  - "Yes — run /hunt:new-program" → tell user to `cd $TARGET_PATH` first, then run `/hunt:new-program`
