...
AGENT_SKILLS_UI=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" resolve-model thrunt-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```bash
PHASE_INFO=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" huntmap get-phase "${PHASE}")
```
...
```markdown
Read ~/.claude/agents/thrunt-ui-researcher.md for instructions.
...
Template: ~/.claude/thrunt-god/templates/UI-SPEC.md
</output>
...
```markdown
Read ~/.claude/agents/thrunt-ui-checker.md for instructions.
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
