...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" agent-skills thrunt-scope-analyzer 2>/dev/null)
```
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
2. Sync chain flag:
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" config-set workflow._auto_chain_active true
```
