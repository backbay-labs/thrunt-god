<purpose>
Cross-phase audit of all Evidence Review and findings validation files. Finds every outstanding item (pending, skipped, blocked, human_needed), optionally validates against the codebase to detect stale docs, and produces a prioritized human test plan.
</purpose>
...
```bash
AUDIT=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" audit-evidence --raw)
```
...
No outstanding Evidence Review or findings validation items found across all phases.
...
- `human_uat` — human validation items
...
1. **Close stale items:** `/hunt:validate-findings {phase}` — mark stale tests as resolved
2. **Run active tests:** Human Evidence Review test plan below
3. **When prerequisites met:** Retest blocked items with `/hunt:validate-findings {phase}`
```
</step>

<step name="test_plan">
Generate a human Evidence Review test plan for "Testable Now" + "active" items only:

Group by what can be tested together (same screen, same feature, same prerequisite):

```
## Human Evidence Review Test Plan
