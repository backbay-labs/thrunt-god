<purpose>
Remove a THRUNT workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/thrunt:list-workspaces` to show available workspaces, then ask:
