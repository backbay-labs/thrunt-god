...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing thrunt-tools.cjs subcommands and the thrunt-analyst-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/thrunt-god/references/ui-brand.md (display patterns)
- @$HOME/.claude/thrunt-god/agents/thrunt-analyst-profiler.md (profiler agent definition)
- @$HOME/.claude/thrunt-god/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/thrunt-god/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/thrunt-god/USER-PROFILE.md" "$HOME/.claude/thrunt-god/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/thrunt-god/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /thrunt:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn thrunt-analyst-profiler agent using Task tool:**

Use the Task tool to spawn the `thrunt-analyst-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/thrunt-god/references/user-profiling.md`
...
Reference: @$HOME/.claude/thrunt-god/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/thrunt-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/thrunt-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/thrunt-god/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/thrunt:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/thrunt-god/USER-PROFILE.md" and jump to step 10.
...
**For /thrunt:dev-preferences (if selected):**

```bash
node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /thrunt:dev-preferences at $HOME/.claude/commands/thrunt/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/thrunt-god/bin/thrunt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any thrunt-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/thrunt-god/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 THRUNT > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/thrunt-god/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /thrunt:dev-preferences   $HOME/.claude/commands/thrunt/dev-preferences.md
...
- [ ] Artifacts generated sequentially via thrunt-tools.cjs subcommands
