...
For each completed milestone without a `-phases` archive, read the archived HUNTMAP snapshot to determine which phases belong to it:

```bash
cat .planning/milestones/v{X.Y}-HUNTMAP.md
```

Extract phase numbers and names from the archived huntmap (e.g., Phase 1: Foundation, Phase 2: Auth).
...
```bash
node "$HOME/.claude/thrunt-god/bin/thrunt-tools.cjs" commit "chore: archive phase directories from completed milestones" --files .planning/milestones/ .planning/phases/
```
...
- [ ] Phase membership determined from archived HUNTMAP snapshots
