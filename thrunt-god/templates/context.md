...
- `thrunt-query-writer` — Reads decisions to focus research (e.g., "card layout" → research card component patterns)
- `thrunt-hunt-planner` — Reads decisions to create specific tasks (e.g., "infinite scroll" → task includes virtualization)
...
[Clear statement of what this phase delivers — the scope anchor. This comes from HUNTMAP.md and is fixed. Discussion clarifies implementation within this boundary.]
...
- `thrunt-query-writer` uses decisions to focus investigation AND reads canonical_refs to know WHAT docs to study
- `thrunt-hunt-planner` uses decisions + research to create executable tasks AND reads canonical_refs to verify alignment
...
- If HUNTMAP.md lists `Canonical refs:` per phase, extract and expand those
