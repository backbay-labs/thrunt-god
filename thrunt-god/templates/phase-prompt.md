...
> **Note:** Planning methodology is in `agents/thrunt-hunt-planner.md`.
...
# Goal-backward validation (derived during planning, validated after execution)
must_haves:
...
@~/.claude/thrunt-god/references/checkpoints.md
</execution_context>
...
<!-- For checkpoint task examples and patterns, see @~/.claude/thrunt-god/references/checkpoints.md -->
...
| `requirements` | Yes | **MUST** list requirement IDs from HUNTMAP. Every huntmap requirement MUST appear in at least one plan. |
| `user_setup` | No | Array of human-required setup items (external services) |
| `must_haves` | Yes | Goal-backward validation criteria (see below) |

**Wave is pre-computed:** Wave numbers are assigned during `/hunt:plan`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.

**Must-haves enable validation:** The `must_haves` field carries goal-backward requirements from planning to execution. After all plans complete, hunt-run spawns a validation subagent that checks these criteria against the actual codebase.
...
See `~/.claude/thrunt-god/references/tdd.md` for TDD plan structure.
...
| `checkpoint:human-verify` | Visual/functional validation | Pauses, returns to orchestrator |
...
See `~/.claude/thrunt-god/templates/user-setup.md` for full schema and examples
...
Task completion ≠ Goal achievement. A task "create chat component" can complete by creating a placeholder. The `must_haves` field captures what must actually work, enabling validation to catch gaps before they compound.
...
4. Validation subagent checks must_haves against codebase
5. Gaps found → fix plans created → execute → re-verify
6. All must_haves pass → phase complete

See `~/.claude/thrunt-god/workflows/findings-validation.md` for validation logic.
