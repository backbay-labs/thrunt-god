 ...
 *
 * Workstreams enable parallel hunts by scoping huntmap/state
 * artifacts into .planning/workstreams/{name}/ directories.
 */
 ...
 * Moves per-workstream hunt artifacts into `.planning/workstreams/{name}/`.
 * Shared files like `MISSION.md`, `config.json`, `environment/`, `milestones/`,
 * `research/`, `codebase/`, and `todos/` stay in place.
 */
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error('.planning/ directory not found — run /hunt:new-case or /hunt:new-program first');
  }
  ...
  if (isFlatMode && options.migrate !== false) {
    const hasExistingWork = fs.existsSync(path.join(baseDir, 'HUNTMAP.md')) ||
                            fs.existsSync(path.join(baseDir, 'STATE.md')) ||
                            fs.existsSync(path.join(baseDir, 'HYPOTHESES.md')) ||
                            fs.existsSync(path.join(baseDir, 'SUCCESS_CRITERIA.md')) ||
                            fs.existsSync(path.join(baseDir, 'FINDINGS.md')) ||
                            fs.existsSync(path.join(baseDir, 'EVIDENCE_REVIEW.md')) ||
                            fs.existsSync(path.join(baseDir, 'phases')) ||
                            fs.existsSync(path.join(baseDir, 'QUERIES')) ||
                            fs.existsSync(path.join(baseDir, 'RECEIPTS')) ||
                            fs.existsSync(path.join(baseDir, 'published'));
...
function cmdWorkstreamList(cwd, raw) {
  ...
  for (const entry of entries) {
    ...
    workstreams.push({
      ...
      has_huntmap: fs.existsSync(path.join(wsDir, 'HUNTMAP.md')),
...
function cmdWorkstreamStatus(cwd, name, raw) {
  ...
  const files = {
    ...
    huntmap: fs.existsSync(p.huntmap),
    hypotheses: fs.existsSync(p.hypotheses),
    success_criteria: fs.existsSync(p.successCriteria),
    findings: fs.existsSync(p.findings),
    evidence_review: fs.existsSync(p.evidenceReview),
    queries: fs.existsSync(p.queries),
    receipts: fs.existsSync(p.receipts),
  };
  ...
  output({
    ...
    path: relPath,
    files,
    query_count: fs.existsSync(p.queries) ? fs.readdirSync(p.queries).length : 0,
    receipt_count: fs.existsSync(p.receipts) ? fs.readdirSync(p.receipts).length : 0,
    phases,
...
function cmdWorkstreamProgress(cwd, raw) {
  ...
  for (const entry of entries) {
    ...
    try {
      const roadmapPath = fs.existsSync(path.join(wsDir, 'HUNTMAP.md'))
        ? path.join(wsDir, 'HUNTMAP.md')
        : path.join(wsDir, 'HUNTMAP.md');
