...
const CLAUDE_MD_FALLBACKS = {
  mission: 'Mission not yet initialized. Run /hunt:new-case or /hunt:new-program to set up.',
...
const CLAUDE_MD_SECTION_ALIASES = {
  mission: ['project'],
};
...
function findManagedSection(fileContent, sectionName) {
  const names = [sectionName, ...(CLAUDE_MD_SECTION_ALIASES[sectionName] || [])];
  for (const name of names) {
    const startMarker = `<!-- THRUNT:${name}-start`;
    const endMarker = `<!-- THRUNT:${name}-end -->`;
    ...
    return {
      ...
      endIdx,
      endMarker,
      content: fileContent.substring(startTagEnd + 3, endIdx),
    };
  }
  return null;
}
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- THRUNT:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- THRUNT:${sectionName}-end -->`,
  ].join('\n');
}

function updateSection(fileContent, sectionName, newContent) {
  const section = findManagedSection(fileContent, sectionName);
  if (section) {
    const before = fileContent.substring(0, section.startIdx);
    const after = fileContent.substring(section.endIdx + section.endMarker.length);
...
function detectManualEdit(fileContent, sectionName, expectedContent) {
  ...
  const currentContent = section.matchedName === 'project' && sectionName === 'mission'
    ? section.content
        .replace(/^## Project$/m, '## Mission')
        .replace(
          /Project or mission not yet initialized\. Run \/hunt:new-case, \/hunt:new-program, or \/hunt:new-program to set up\./,
          CLAUDE_MD_FALLBACKS.mission
        )
    : section.content;
...
function generateMissionSection(cwd) {
  const projectDoc = getMissionDocInfo(cwd);
  const content = safeReadFile(projectDoc.path);
  if (!content) {
    return { content: CLAUDE_MD_FALLBACKS.mission, source: 'MISSION.md', hasFallback: true };
  }

  if (projectDoc.source === 'MISSION.md') {
    const parts = [];
    const h1Match = content.match(/^# (.+)$/m);
    if (h1Match) parts.push(`**${h1Match[1]}**`);

    const desiredOutcome = extractMarkdownSection(content, 'Desired Outcome');
    if (desiredOutcome) {
      const body = desiredOutcome.replace(/^## Desired Outcome\s*/i, '').trim();
      if (body) parts.push(body);
    }

    const signal = extractMarkdownSection(content, 'Signal');
    if (signal) {
      const body = signal.replace(/^## Signal\s*/i, '').trim();
      if (body) parts.push(`**Signal:** ${body}`);
    }

    const workingTheory = extractMarkdownSection(content, 'Working Theory');
    if (workingTheory) {
      const body = workingTheory.replace(/^## Working Theory\s*/i, '').trim();
      if (body) parts.push(`**Working Theory:** ${body}`);
    }

    const constraints = extractMarkdownSection(content, 'Operating Constraints');
    if (constraints) {
      const body = constraints.replace(/^## Operating Constraints\s*/i, '').trim();
    ...
    return { content: parts.join('\n\n'), source: 'MISSION.md', hasFallback: false };
  }
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'thrunt-god', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/thrunt:profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'thrunt', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/thrunt:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by THRUNT from ${dataSource}. Run \`/thrunt:profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- THRUNT:profile-start -->';
    const endMarker = '<!-- THRUNT:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  const MANAGED_SECTIONS = ['mission', 'stack', 'conventions', 'architecture', 'workflow'];
  const generators = {
    mission: generateMissionSection,
  ...
  const sectionHeadings = {
    ...
    workflow: '## THRUNT Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = findManagedSection(fileContent, name) !== null;
    ...
    if (!options.auto && fileContent.indexOf('<!-- THRUNT:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- THRUNT:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- THRUNT:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /thrunt:profile-user to unlock Developer Profile.';
