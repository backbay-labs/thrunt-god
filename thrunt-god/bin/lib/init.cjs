...
function relToCwd(cwd, filePath) {
  return toPosixPath(path.relative(cwd, filePath));
}

function cmdInitRun(cwd, phase, raw) {
  if (!phase) {
    error('phase required for init run');
  }
  ...
  const huntmapPhase = getHuntmapPhaseInternal(cwd, phase);

  // Fallback to HUNTMAP.md if no phase directory exists yet
  if (!phaseInfo && huntmapPhase?.found) {
    const phaseName = huntmapPhase.phase_name;
    phaseInfo = {
      found: true,
      directory: null,
      phase_number: huntmapPhase.phase_number,
  ...
  const reqMatch = huntmapPhase?.section?.match(/^\*\*Hypotheses\*\*:[^\S\n]*([^\n]*)$/m);
  const reqExtracted = reqMatch
    ? reqMatch[1].replace(/[\[\]]/g, '').split(',').map(s => s.trim()).filter(Boolean).join(', ')
    : null;
  const phase_hypothesis_ids = (reqExtracted && reqExtracted !== 'TBD') ? reqExtracted : null;

  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'thrunt-telemetry-executor'),
    validator_model: resolveModelInternal(cwd, 'thrunt-findings-validator'),
    ...
    validator_enabled: config.validator,
    ...
    // File paths
    state_path: relToCwd(cwd, path.join(planningDir(cwd), 'STATE.md')),
    huntmap_path: relToCwd(cwd, roadmapDoc.path),
    huntmap_source: roadmapDoc.source,
    config_path: relToCwd(cwd, path.join(planningDir(cwd), 'config.json')),
  };

  output(withProjectRoot(cwd, result), raw);
}

function cmdInitPlan(cwd, phase, raw) {
  if (!phase) {
    error('phase required for init plan');
  }
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'thrunt-query-writer'),
    planner_model: resolveModelInternal(cwd, 'thrunt-hunt-planner'),
    checker_model: resolveModelInternal(cwd, 'thrunt-hunt-checker'),

    // Workflow flags
    research_enabled: config.research,
    plan_check_enabled: config.plan_check,
    ...
    huntmap_path: relToCwd(cwd, roadmapDoc.path),
    huntmap_source: roadmapDoc.source,
    hypotheses_path: relToCwd(cwd, path.join(planningDir(cwd), 'HYPOTHESES.md')),
  };
  ...
  if (phaseInfo?.directory) {
    ...
    try {
      ...
      const verificationFile = files.find(f => f.endsWith('-FINDINGS.md') || f === 'FINDINGS.md');
      if (verificationFile) {
        result.findings_path = toPosixPath(path.join(phaseInfo.directory, verificationFile));
      }
      const uatFile = files.find(f => f.endsWith('-EVIDENCE_REVIEW.md') || f === 'EVIDENCE_REVIEW.md');
      if (uatFile) {
        result.evidence_review_path = toPosixPath(path.join(phaseInfo.directory, uatFile));
      }
...
function cmdInitNewProgram(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'thrunt-signal-triager'),
    synthesizer_model: resolveModelInternal(cwd, 'thrunt-intel-synthesizer'),
    huntmap_builder_model: resolveModelInternal(cwd, 'thrunt-huntmap-builder'),
    ...
    // File paths
    mission_path: relToCwd(cwd, projectDoc.path),
    mission_source: projectDoc.source,
  };
...
function cmdInitValidateFindings(cwd, phase, raw) {
  if (!phase) {
    error('phase required for init validate-findings');
  }
  ...
  const result = {
    ...
    // Existing artifacts
    has_findings: phaseInfo?.has_findings || false,
  };
...
function cmdInitPhaseOp(cwd, phase, raw) {
  ...
  // current milestone's ROADMAP entry so shape-hypothesis and similar flows
  // don't attach to published work that reused the same phase number.
  ...
  // Fallback to HUNTMAP.md if no directory exists (e.g., Plans: TBD)
  if (!phaseInfo) {
    const huntmapPhase = getHuntmapPhaseInternal(cwd, phase);
    if (huntmapPhase?.found) {
      const phaseName = huntmapPhase.phase_name;
...
function cmdInitMapEnvironment(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'thrunt-environment-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  const workflowMode = 'hunt';
  const expectedRoadmapDoc = 'HUNTMAP.md';
  const bootstrapHint = 'Run /hunt:new-case or /hunt:new-program first.';

  function getManagerCommand(action, phaseNumber) {
    if (action === 'execute') return `/hunt:run ${phaseNumber}`;
    if (action === 'plan') return `/hunt:plan ${phaseNumber}`;
    return `/hunt:shape-hypothesis ${phaseNumber}`;
  }

  // Validate prerequisites
  if (!roadmapDoc.exists) {
    error(`No ${expectedRoadmapDoc} found. ${bootstrapHint}`);
  }
  if (!fs.existsSync(paths.state)) {
    error(`No STATE.md found. ${bootstrapHint}`);
  }
  const rawContent = fs.readFileSync(roadmapDoc.path, 'utf-8');
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: getManagerCommand('execute', phase.number),
      });
      ...
      recommendedActions.push({
        ...
        command: getManagerCommand('plan', phase.number),
      });
      ...
      recommendedActions.push({
        ...
        command: getManagerCommand('discuss', phase.number),
      });
    }
  }
  ...
  function hasDependencyRelationship(numA, numB) {
  ...
  const filteredActions = recommendedActions.filter(action => {
    if (action.action === 'execute' && activeExecuting.length > 0) {
      ...
      return activeExecuting.every(active => !hasDependencyRelationship(action.phase, active.number));
    }
    if (action.action === 'plan' && activePlanning.length > 0) {
      // Only allow if independent of ALL actively-planning phases
      return activePlanning.every(active => !hasDependencyRelationship(action.phase, active.number));
    }
    return true;
  });
...
function cmdInitProgress(cwd, raw) {
  ...
  const huntmapPhaseNums = new Set();
  const huntmapPhaseNames = new Map();
  try {
    const roadmapContent = extractCurrentMilestone(
      fs.readFileSync(roadmapDoc.path, 'utf-8'), cwd
    );
    const headingPattern = /#{2,4}\s*Phase\s+(\d+[A-Z]?(?:\.\d+)*)\s*:\s*([^\n]+)/gi;
    let hm;
    while ((hm = headingPattern.exec(roadmapContent)) !== null) {
      huntmapPhaseNums.add(hm[1]);
      huntmapPhaseNames.set(hm[1], hm[2].replace(/\(INSERTED\)/i, '').trim());
    }
  } catch { /* intentionally empty */ }
  ...
  for (const [num, name] of huntmapPhaseNames) {
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'thrunt-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'thrunt-telemetry-executor', 'thrunt-hunt-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills thrunt-telemetry-executor 2>/dev/null)
 */
...
module.exports = {
  ...
  cmdInitQuick,
  cmdInitResume,
  cmdInitValidateFindings,
  cmdInitPhaseOp,
  cmdInitTodos,
  cmdInitMilestoneOp,
  cmdInitMapEnvironment,
  cmdInitProgress,
  cmdInitManager,
  cmdInitNewWorkspace,
