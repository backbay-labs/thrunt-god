/**
 * THRUNT Workspace Tests
 *
 * Tests for /thrunt:new-workspace, /thrunt:list-workspaces, /thrunt:remove-workspace
 * init functions and integration with thrunt-tools routing.
 */
...
const { runThruntTools, createTempDir, cleanup } = require('./helpers.cjs');
const { detectChildRepos } = require('../thrunt-god/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('thrunt-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via thrunt-tools ──────────────────────────────────────
...
describe('init new-workspace', () => {
  ...
  test('returns expected JSON fields', () => {
    const result = runThruntTools('init new-workspace', tmpDir);
...
// ─── cmdInitListWorkspaces via thrunt-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('returns empty list when no workspaces exist', () => {
    const result = runThruntTools('init list-workspaces', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'thrunt-workspaces');
...
// ─── cmdInitRemoveWorkspace via thrunt-tools ───────────────────────────────────
...
describe('init remove-workspace', () => {
  ...
  test('errors when no name provided', () => {
    const result = runThruntTools('init remove-workspace', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('workspace name required'));
  });

  test('errors when workspace not found', () => {
    const result = runThruntTools('init remove-workspace nonexistent', tmpDir, { HOME: tmpDir });
    assert.strictEqual(result.success, false);
  ...
  test('returns workspace info for existing workspace', () => {
    ...
    const result = runThruntTools('init remove-workspace test-ws', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('thrunt-ws-integ-');
...
describe('workspace command files', () => {
  ...
  test('new-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/thrunt/new-workspace.md'), 'utf8');
    assert.ok(content.includes('name: thrunt:new-workspace'));
  ...
  test('list-workspaces command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/thrunt/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('name: thrunt:list-workspaces'));
    assert.ok(content.includes('workflows/list-workspaces.md'));
  });

  test('remove-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/thrunt/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('name: thrunt:remove-workspace'));
    assert.ok(content.includes('workflows/remove-workspace.md'));
  });

  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'thrunt-god/workflows/new-workspace.md'), 'utf8');
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'thrunt-god/workflows/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('init list-workspaces'));
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'thrunt-god/workflows/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('init remove-workspace'));
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in thrunt-tools ───────────────────────────────────────────────────

describe('workspace routing in thrunt-tools', () => {
  test('init new-workspace is routed correctly', () => {
    const toolsContent = fs.readFileSync(
      path.join(__dirname, '..', 'thrunt-god', 'bin', 'thrunt-tools.cjs'),
      'utf8'
    );
