...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    const result = runThruntTools('validate consistency', tmpDir);
  ...
  test('warns about phase on disk but not in roadmap', () => {
    ...
    assert.ok(
      output.warnings.some(w => w.includes('disk but not in HUNTMAP')),
  ...
  test('uses HUNTMAP.md in consistency warnings when hunt roadmap is active', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      `# Huntmap\n### Phase 1: Intake\n`
    );
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases', '01-intake'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases', '02-orphan'), { recursive: true });
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some(w => w.includes('HUNTMAP.md')),
      `should use hunt-native roadmap label: ${JSON.stringify(output.warnings)}`
    );
  });
});

// ─────────────────────────────────────────────────────────────────────────────
// validate plan-structure command
// ─────────────────────────────────────────────────────────────────────────────

describe('validate plan-structure command', () => {
  let tmpDir;
  ...
  test('reports missing required frontmatter fields', () => {
    ...
    const result = runThruntTools('validate plan-structure .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runThruntTools('validate plan-structure .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// validate phase-completeness command
// ─────────────────────────────────────────────────────────────────────────────

describe('validate phase-completeness command', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempProject();
    // Create HUNTMAP.md referencing phase 01 so findPhaseInternal can locate it
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      '# Huntmap\n\n### Phase 1: Test\n**Goal**: Test phase\n'
    );
  ...
  test('reports complete phase with matching plans and summaries', () => {
    ...
    const result = runThruntTools('validate phase-completeness 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent phase', () => {
    const result = runThruntTools('validate phase-completeness 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('validate summary command', () => {
  let tmpDir;
  ...
  test('returns not found for nonexistent summary', () => {
    const result = runThruntTools('validate-summary .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const result = runThruntTools('validate-summary .planning/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runThruntTools('validate-summary .planning/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('validate references command', () => {
  let tmpDir;
  ...
  test('reports valid when all referenced files exist', () => {
    ...
    const result = runThruntTools('validate references .planning/phases/01-test/doc.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runThruntTools('validate references .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// validate commits command
// ─────────────────────────────────────────────────────────────────────────────

describe('validate commits command', () => {
  let tmpDir;
  ...
  test('validates real commit hashes', () => {
    ...
    const result = runThruntTools(`validate commits ${hash}`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports invalid for fake hashes', () => {
    const result = runThruntTools('validate commits abcdef1234567', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles mixed valid and invalid hashes', () => {
    ...
    const result = runThruntTools(`validate commits ${hash} abcdef1234567`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('validate artifacts command', () => {
  let tmpDir;
  ...
  test('passes when all artifacts exist and match criteria', () => {
    ...
    const result = runThruntTools('validate artifacts .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// validate key-links command
// ─────────────────────────────────────────────────────────────────────────────

describe('validate key-links command', () => {
  let tmpDir;
  ...
  test('verifies link when pattern found in source', () => {
    ...
    const result = runThruntTools('validate key-links .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
