 ...
 *
 * Validates the /hunt:run --wave feature contract:
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'hunt', 'run.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'thrunt-god', 'workflows', 'hunt-run.md');
const COMMANDS_DOC_PATH = path.join(__dirname, '..', 'docs', 'COMMANDS.md');
const HELP_PATH = path.join(__dirname, '..', 'thrunt-god', 'workflows', 'help.md');

describe('hunt run command: --wave flag', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/hunt/run.md should exist');
  });
...
describe('hunt run workflow: wave filtering', () => {
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WORKFLOW_PATH), 'workflows/hunt-run.md should exist');
  });
...
describe('hunt run docs: user-facing wave flag', () => {
  test('COMMANDS.md documents --wave usage', () => {
    ...
    assert.ok(
      content.includes('/hunt:run 1 --wave 2'),
  ...
  test('help workflow documents --wave behavior', () => {
    ...
    assert.ok(
      content.includes('Usage: `/hunt:run 5 --wave 2`'),
