/**
 * THRUNT Tools Tests - Dispatcher
 *
 * Tests for thrunt-tools.cjs dispatch routing and error paths.
...
describe('dispatcher error paths', () => {
  ...
  test('no-command invocation prints usage and exits non-zero', () => {
    const result = runThruntTools('', tmpDir);
  ...
  test('unknown command produces clear error and exits non-zero', () => {
    const result = runThruntTools('nonexistent-cmd', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd= form overrides working directory', () => {
    ...
    const result = runThruntTools(`--cwd=${tmpDir} state load`, process.cwd());
  ...
  test('--cwd= with empty value produces error', () => {
    const result = runThruntTools('--cwd= state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd with invalid path produces error', () => {
    const result = runThruntTools('--cwd /nonexistent/path/xyz state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('template unknown subcommand errors', () => {
    const result = runThruntTools('template bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('frontmatter unknown subcommand errors', () => {
    const result = runThruntTools('frontmatter bogus file.md', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('validate unknown subcommand errors', () => {
    const result = runThruntTools('validate bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phases unknown subcommand errors', () => {
    const result = runThruntTools('phases bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
    assert.ok(result.error.includes('Unknown phases subcommand'), `Expected "Unknown phases subcommand" in stderr, got: ${result.error}`);
  });

  // Unknown subcommand: huntmap
  test('huntmap unknown subcommand errors', () => {
    const result = runThruntTools('huntmap bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
    assert.ok(result.error.includes('Unknown huntmap subcommand'), `Expected "Unknown huntmap subcommand" in stderr, got: ${result.error}`);
  });

  // Unknown subcommand: hypotheses
  test('hypotheses unknown subcommand errors', () => {
    const result = runThruntTools('hypotheses bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
    assert.ok(result.error.includes('Unknown hypotheses subcommand'), `Expected "Unknown hypotheses subcommand" in stderr, got: ${result.error}`);
  });

  // Unknown subcommand: phase
  test('phase unknown subcommand errors', () => {
    const result = runThruntTools('phase bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('milestone unknown subcommand errors', () => {
    const result = runThruntTools('milestone bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('todo unknown subcommand errors', () => {
    const result = runThruntTools('todo bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
    assert.ok(result.error.includes('Unknown todo subcommand'), `Expected "Unknown todo subcommand" in stderr, got: ${result.error}`);
  });

  test('evidence unknown subcommand errors', () => {
    const result = runThruntTools('evidence bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
    assert.ok(result.error.includes('Unknown evidence subcommand'), `Expected "Unknown evidence subcommand" in stderr, got: ${result.error}`);
  });

  // Unknown subcommand: init
  test('init unknown workflow errors', () => {
    const result = runThruntTools('init bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
...
describe('dispatcher routing branches', () => {
  ...
  test('find-phase locates phase directory by number', () => {
    ...
    const result = runThruntTools('find-phase 01', tmpDir);
  ...
  test('init resume returns valid JSON', () => {
    ...
    const result = runThruntTools('init resume', tmpDir);
  ...
  // init validate-findings
  test('init validate-findings returns valid JSON', () => {
    // Create STATE.md
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '# Hunt State\n\n## Current Position\n\nPhase: 1 of 1 (Test)\nPlan: 01-01 complete\nStatus: Ready\nLast activity: 2026-01-01\n\nProgress: [##########] 100%\n\n## Session Continuity\n\nLast session: 2026-01-01\nStopped at: Test\nResume file: None\n'
    ...
    // Create HUNTMAP.md with phase section
    fs.writeFileSync(
    ...
    const result = runThruntTools('init validate-findings 01', tmpDir);
    assert.strictEqual(result.success, true, `init validate-findings failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
    assert.ok(typeof parsed === 'object', 'Output should be valid JSON object');
  });

  // huntmap update-plan-progress
  test('huntmap update-plan-progress updates phase progress', () => {
    // Create HUNTMAP.md with progress table
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      '# Huntmap\n\n## Milestone: v1.0 Test\n\n### Phase 1: Test Phase\n**Goal**: Test goal\n**Depends on**: None\n**Hypotheses**: TEST-01\n**Success Criteria**:\n  1. Tests pass\n**Plans**: 1 plan\nPlans:\n- [ ] 01-01-PLAN.md\n\n## Progress\n\n| Phase | Plans | Status | Date |\n|-------|-------|--------|------|\n| 1 | 0/1 | Not Started | - |\n'
    );
    ...
    assert.strictEqual(result.success, true, `huntmap update-plan-progress failed: ${result.error}`);
  });
  ...
  test('state with no subcommand calls cmdStateLoad', () => {
    ...
    const result = runThruntTools('state', tmpDir);
...
`;
    ...
    assert.deepStrictEqual(parsed.hypotheses_completed, ['TEST-01'], 'hypotheses_completed should contain TEST-01');
  });
});
