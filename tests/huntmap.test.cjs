...
describe('huntmap get-phase command', () => {
  let tmpDir;
  ...
  test('extracts phase section from HUNTMAP.md', () => {
    fs.writeFileSync(
...
`
    ...
    const result = runThruntTools('huntmap get-phase 1', tmpDir);
  ...
  test('extracts phase section from HUNTMAP.md when HUNTMAP is absent', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      `# Huntmap: Identity Escalation Hunt

### Phase 1: Signal Intake
**Goal**: Clarify the suspicious admin action
**Plans:** 2 plans

### Phase 2: Swarm Execution
**Goal**: Query identity, endpoint, and cloud telemetry
`
    );
    ...
    assert.strictEqual(output.goal, 'Clarify the suspicious admin action');
  });
...
`
    ...
    const result = runThruntTools('huntmap get-phase 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runThruntTools('huntmap get-phase 2.1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
  ...
  test('handles missing HUNTMAP.md gracefully', () => {
    ...
    assert.strictEqual(output.error, 'HUNTMAP.md not found', 'should explain why');
  });

  test('uses HUNTMAP label when mission exists but no roadmap doc exists', () => {
    fs.writeFileSync(
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.found, false, 'should return not found');
    assert.strictEqual(output.error, 'HUNTMAP.md not found', 'should explain the missing hunt roadmap');
  });
...
`
    ...
    const result1 = runThruntTools('huntmap get-phase 1', tmpDir);
    ...
    const result2 = runThruntTools('huntmap get-phase 2', tmpDir);
  ...
  test('detects malformed HUNTMAP with summary list but no detail sections', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      `# Huntmap v1.0
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.found, false, 'phase should not be found');
    assert.strictEqual(output.error, 'malformed_huntmap', 'should identify malformed roadmap');
    assert.ok(output.message.includes('missing'), 'should explain the issue');
  });

  test('detects malformed HUNTMAP with hunt-native label in the message', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      `# Huntmap

- [ ] **Phase 1: Signal Intake** - Intake suspicious activity
`
    );
    ...
    assert.strictEqual(output.found, false, 'phase should not be found');
    assert.strictEqual(output.error, 'malformed_huntmap', 'should identify malformed roadmap');
    assert.ok(output.message.includes('HUNTMAP.md needs both formats'), 'should point at HUNTMAP.md');
  });
});
...
describe('huntmap analyze command', () => {
  let tmpDir;
  ...
  test('missing HUNTMAP.md returns error', () => {
    const result = runThruntTools('huntmap analyze', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.error, 'HUNTMAP.md not found');
  });

  test('uses HUNTMAP label when mission exists but huntmap is missing', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'MISSION.md'),
      '# Mission\n\nThreat hunt mission.\n'
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// huntmap analyze disk status variants
// ─────────────────────────────────────────────────────────────────────────────

describe('huntmap analyze disk status variants', () => {
  let tmpDir;
...
// ─────────────────────────────────────────────────────────────────────────────
// huntmap analyze milestone extraction
// ─────────────────────────────────────────────────────────────────────────────

describe('huntmap analyze milestone extraction', () => {
  let tmpDir;
...
// ─────────────────────────────────────────────────────────────────────────────
// huntmap analyze missing phase details
// ─────────────────────────────────────────────────────────────────────────────

describe('huntmap analyze missing phase details', () => {
  let tmpDir;
...
// ─────────────────────────────────────────────────────────────────────────────
// huntmap get-phase success criteria
// ─────────────────────────────────────────────────────────────────────────────

describe('huntmap get-phase success criteria', () => {
  let tmpDir;
...
// ─────────────────────────────────────────────────────────────────────────────
// huntmap update-plan-progress command
// ─────────────────────────────────────────────────────────────────────────────

describe('huntmap update-plan-progress command', () => {
  let tmpDir;
  ...
  test('missing phase number returns error', () => {
    const result = runThruntTools('huntmap update-plan-progress', tmpDir);
...
`
    ...
    const result = runThruntTools('huntmap update-plan-progress 99', tmpDir);
...
`
    ...
    const result = runThruntTools('huntmap update-plan-progress 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const roadmapContent = fs.readFileSync(path.join(tmpDir, '.planning', 'HUNTMAP.md'), 'utf-8');
...
`
  ...
  test('missing HUNTMAP.md returns updated false', () => {
    // Create phase dir with plans and summaries but NO HUNTMAP.md
    const p1 = path.join(tmpDir, '.planning', 'phases', '01-test');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan 1');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary 1');
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.updated, false, 'should not update');
    assert.ok(output.reason.includes('HUNTMAP.md not found'), 'reason should mention missing HUNTMAP.md');
  });

  test('uses HUNTMAP label when mission exists but roadmap doc is missing', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'MISSION.md'),
      '# Mission\n\nThreat hunt mission.\n'
    );
...
`;
    fs.writeFileSync(path.join(tmpDir, '.planning', 'HUNTMAP.md'), roadmapContent);
    ...
    const result = runThruntTools('huntmap update-plan-progress 50', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
