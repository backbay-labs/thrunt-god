/**
 * THRUNT Tools Tests - frontmatter CLI integration
 *
 * Integration tests for the 4 frontmatter subcommands (get, set, merge, validate)
 * exercised through thrunt-tools.cjs via execSync.
 *
...
function writeTempFile(content) {
  const tmpFile = path.join(os.tmpdir(), `thrunt-fm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.md`);
...
describe('frontmatter get', () => {
  test('returns all fields as JSON', () => {
    ...
    const result = runThruntTools(`frontmatter get ${file}`);
  ...
  test('returns specific field with --field', () => {
    const file = writeTempFile('---\nphase: 01\nplan: 02\ntype: tdd\n---\nbody');
    const result = runThruntTools(`frontmatter get ${file} --field phase`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.phase, '01');
  });
  ...
  test('returns error for missing field', () => {
    const file = writeTempFile('---\nphase: 01\n---\n');
    const result = runThruntTools(`frontmatter get ${file} --field nonexistent`);
  ...
  test('returns error for missing file', () => {
    const result = runThruntTools('frontmatter get /nonexistent/path/file.md');
...
describe('frontmatter set', () => {
  test('updates existing field', () => {
    const file = writeTempFile('---\nphase: 01\ntype: execute\n---\nbody');
    const result = runThruntTools(`frontmatter set ${file} --field phase --value "02"`);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('adds new field', () => {
    const file = writeTempFile('---\nphase: 01\n---\nbody');
    const result = runThruntTools(`frontmatter set ${file} --field status --value "active"`);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles JSON array value', () => {
    const file = writeTempFile('---\nphase: 01\n---\nbody');
    const result = runThruntTools(['frontmatter', 'set', file, '--field', 'tags', '--value', '["a","b"]']);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for missing file', () => {
    const result = runThruntTools('frontmatter set /nonexistent/file.md --field phase --value "01"');
    assert.ok(result.success, 'Command should exit 0 with error JSON');
    const parsed = JSON.parse(result.output);
    assert.ok(parsed.error, 'Should have error field');
  });
  ...
  test('preserves body content after set', () => {
    ...
    const file = writeTempFile('---\nphase: 01\n---' + bodyText);
    runThruntTools(`frontmatter set ${file} --field phase --value "02"`);
...
describe('frontmatter merge', () => {
  test('merges multiple fields into frontmatter', () => {
    const file = writeTempFile('---\nphase: 01\n---\nbody');
    const result = runThruntTools(['frontmatter', 'merge', file, '--data', '{"plan":"02","type":"tdd"}']);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('overwrites existing fields on conflict', () => {
    const file = writeTempFile('---\nphase: 01\ntype: execute\n---\nbody');
    const result = runThruntTools(['frontmatter', 'merge', file, '--data', '{"phase":"02"}']);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for missing file', () => {
    const result = runThruntTools(`frontmatter merge /nonexistent/file.md --data '{"phase":"01"}'`);
    assert.ok(result.success, 'Command should exit 0 with error JSON');
    const parsed = JSON.parse(result.output);
    assert.ok(parsed.error, 'Should have error field');
  });
  ...
  test('returns error for invalid JSON data', () => {
    const file = writeTempFile('---\nphase: 01\n---\nbody');
    const result = runThruntTools(`frontmatter merge ${file} --data 'not json'`);
...
body`;
    ...
    const result = runThruntTools(`frontmatter validate ${file} --schema plan`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
...
body`;
    const file = writeTempFile(content);
    const result = runThruntTools(`frontmatter validate ${file} --schema summary`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
  ...
  test('validates against findings schema', () => {
    const content = `---
phase: 01
verified: 2026-02-25
status: passed
...
body`;
    const file = writeTempFile(content);
    const result = runThruntTools(`frontmatter validate ${file} --schema findings`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.valid, true, 'Should be valid for findings schema');
    assert.strictEqual(parsed.schema, 'findings');
  });

  test('returns error for unknown schema', () => {
    const file = writeTempFile('---\nphase: 01\n---\n');
    const result = runThruntTools(`frontmatter validate ${file} --schema unknown`);
  ...
  test('returns error for missing file', () => {
    const result = runThruntTools('frontmatter validate /nonexistent/file.md --schema plan');
    assert.ok(result.success, 'Command should exit 0 with error JSON');
    const parsed = JSON.parse(result.output);
    assert.ok(parsed.error, 'Should have error field');
  });
