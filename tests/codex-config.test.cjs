/**
 * THRUNT Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('hunt-run');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('hunt-plan');
    assert.ok(result.includes('`$hunt-plan`'), 'has $skillName invocation');
    assert.ok(result.includes('{{THRUNT_ARGS}}'), 'has THRUNT_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('hunt-shape-hypothesis');
...
<role>
You are a THRUNT plan executor.
</role>`;
    ...
    assert.ok(result.includes('"thrunt-telemetry-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes THRUNT plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: thrunt-telemetry-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes THRUNT plans with atomic commits'), 'purpose from description');
...
Run /hunt:run to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$hunt-run'), 'converts slash commands');
    assert.ok(!result.includes('/hunt:run'), 'original slash command removed');
  });
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('thrunt-telemetry-executor', sampleAgent);
    assert.ok(result.includes('name = "thrunt-telemetry-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('thrunt-unknown', minimalAgent);
    assert.ok(result.includes('name = "thrunt-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "THRUNT agent thrunt-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('thrunt-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'thrunt-telemetry-executor', 'thrunt-hunt-planner', 'thrunt-query-writer',
      'thrunt-signal-triager', 'thrunt-intel-synthesizer', 'thrunt-findings-validator',
      'thrunt-environment-mapper', 'thrunt-huntmap-builder', 'thrunt-incident-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['thrunt-hunt-checker', 'thrunt-evidence-correlator'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'thrunt-telemetry-executor', description: 'Executes plans' },
    { name: 'thrunt-hunt-planner', description: 'Creates plans' },
  ];

  test('starts with THRUNT marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(THRUNT_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.thrunt-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.thrunt-telemetry-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.thrunt-hunt-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/thrunt-telemetry-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/thrunt-telemetry-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/thrunt-hunt-planner.toml"'), 'absolute planner path');
...
describe('stripThruntFromCodexConfig', () => {
  test('returns null for THRUNT-only config', () => {
    const content = `${THRUNT_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
    const result = stripThruntFromCodexConfig(content);
    assert.strictEqual(result, null, 'returns null when THRUNT-only');
  });
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes THRUNT content');
    assert.ok(!result.includes(THRUNT_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(THRUNT_CODEX_MARKER), 'strips marker');
  });

  test('removes [agents.thrunt-*] sections', () => {
    const content = `[agents.thrunt-telemetry-executor]\ndescription = "test"\nconfig_file = "agents/thrunt-telemetry-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripThruntFromCodexConfig(content);
    assert.ok(!result.includes('[agents.thrunt-telemetry-executor]'), 'removes THRUNT agent section');
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'thrunt-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(THRUNT_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.thrunt-telemetry-executor]'), 'has agent');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing THRUNT block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'thrunt-telemetry-executor', description: 'Updated description' },
      { name: 'thrunt-hunt-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.thrunt-hunt-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(THRUNT_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without THRUNT marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(THRUNT_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.thrunt-telemetry-executor]'), 'has agent');
  });

  test('case 3 with existing [features]: preserves user features, does not inject THRUNT keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(THRUNT_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.thrunt-telemetry-executor]'), 'has agent');
  });

  test('case 3 strips existing [agents.thrunt-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
      ...
      '',
      '',
      '[agents.thrunt-telemetry-executor]',
      'description = "old"',
      'config_file = "agents/thrunt-telemetry-executor.toml"',
      '',
    ].join('\n');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const thruntAgentCount = (content.match(/^\[agents\.thrunt-telemetry-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(THRUNT_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-THRUNT agent section');
    assert.strictEqual(thruntAgentCount, 1, 'keeps exactly one THRUNT agent section');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# THRUNT Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.thrunt-telemetry-executor]'), 'has agent from fresh block');
  });

  test('case 2 strips leaked [agents] and [agents.thrunt-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
    ...
    assert.ok(!beforeMarker.includes('[agents.thrunt-'), 'no leaked [agents.thrunt-*] above marker');
  });

  test('case 2 strips leaked THRUNT-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.thrunt-telemetry-executor\]\s*$/gm), 0, 'removes leaked THRUNT agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.thrunt-telemetry-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked THRUNT sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.thrunt-telemetry-executor\]\s*$/gm), 0, 'removes leaked THRUNT agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.thrunt-telemetry-executor\]\s*$/gm), 1, 'keeps one managed agent section in the THRUNT block');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending THRUNT block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${THRUNT_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending THRUNT block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'thrunt-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(THRUNT_CODEX_MARKER), 'has THRUNT marker');
    assert.ok(config.includes('[agents.thrunt-telemetry-executor]'), 'has executor agent');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'thrunt-telemetry-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'thrunt-hunt-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'thrunt-telemetry-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "thrunt-telemetry-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes THRUNT plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by hunt-run orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'thrunt-hunt-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "thrunt-hunt-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'thrunt-codex-e2e-'));
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# THRUNT Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes THRUNT SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'writes one THRUNT update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'adds one THRUNT update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. THRUNT must not prepend [features] before
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.thrunt-environment-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'does not duplicate THRUNT update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'keeps one THRUNT update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'remains idempotent for the THRUNT hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'adds one THRUNT update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 0, 'does not add the THRUNT hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.thrunt-telemetry-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'adds one THRUNT update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-THRUNT hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-THRUNT hook exactly once');
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'keeps one THRUNT update hook');
    assertUsesOnlyEol(content, '\r\n');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'adds the THRUNT update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# THRUNT Agent Configuration — managed by thrunt-god installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# THRUNT Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the THRUNT hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /thrunt-update-check\.js/g), 1, 'does not duplicate the THRUNT hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'thrunt-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('fresh install removes the THRUNT-added codex_hooks feature on uninstall', () => {
    runCodexInstall(codexHome);

    const cleaned = stripThruntFromCodexConfig(readCodexConfig(codexHome));
    assert.strictEqual(cleaned, null, 'fresh THRUNT-only config strips back to nothing');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^codex_hooks = true$/gm), 0, 'removes the THRUNT-added codex_hooks key');
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-THRUNT hooks');
    assert.strictEqual(countMatches(cleaned, /thrunt-update-check\.js/g), 0, 'removes only the THRUNT update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.thrunt-/g), 0, 'removes managed THRUNT agent sections');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^features\.codex_hooks = true$/gm), 0, 'removes the dotted THRUNT codex_hooks key');
    assert.strictEqual(countMatches(cleaned, /^\[features\]\s*$/gm), 0, 'does not leave behind a [features] table');
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-THRUNT hooks');
    assert.strictEqual(countMatches(cleaned, /thrunt-update-check\.js/g), 0, 'removes the THRUNT update hook');
  });
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing THRUNT hook state', () => {
