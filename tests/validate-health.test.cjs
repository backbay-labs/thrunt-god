/**
 * THRUNT Tools Tests - Validate Health Command
 *
...
// ─── Helpers for setting up minimal valid hunt workspaces ────────────────────

function writeMinimalMissionMd(tmpDir, sections = ['## Signal', '## Desired Outcome', '## Scope']) {
...
function writeMinimalHuntmap(tmpDir, phases = ['1']) {
  const lines = phases.map(n => `### Phase ${n}: Phase ${n} Hunt`).join('\n');
  fs.writeFileSync(
...
describe('validate health command', () => {
  ...
  test('warns when MISSION.md is missing', () => {
    // No MISSION.md in .planning
  ...
  test('reports missing MISSION.md in hunt mode with hunt-native fix guidance', () => {
    writeMinimalHuntmap(tmpDir, ['1']);
    writeMinimalStateMd(tmpDir);
    writeValidConfigJson(tmpDir);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases', '01-a'), { recursive: true });
    ...
    const output = JSON.parse(result.output);
    const e002 = output.errors.find(e => e.code === 'E002');
    assert.ok(e002, `Expected E002 in errors: ${JSON.stringify(output.errors)}`);
    assert.ok(e002.message.includes('MISSION.md'));
    assert.ok(e002.fix.includes('/hunt:new-case'));
  });

  test('warns when MISSION.md missing required sections', () => {
    // MISSION.md missing "## Scope" section
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'MISSION.md'),
      '# Mission: Threat Hunt\n\n## Signal\n\nFoo\n\n## Desired Outcome\n\nBar\n'
    );
    ...
    assert.ok(
      w001s.some(w => w.message.includes('## Scope')),
      `Expected W001 mentioning "## Scope": ${JSON.stringify(w001s)}`
    );
  });

  test('passes when MISSION.md has all required sections', () => {
  ...
  test('passes when MISSION.md and HUNTMAP.md are the active planning docs', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalHuntmap(tmpDir, ['1']);
    writeMinimalStateMd(tmpDir);
    writeValidConfigJson(tmpDir);
    ...
    const output = JSON.parse(result.output);
    assert.ok(!output.errors.some(e => e.code === 'E002'), `Should not have E002: ${JSON.stringify(output.errors)}`);
    assert.ok(!output.errors.some(e => e.code === 'E003'), `Should not have E003: ${JSON.stringify(output.errors)}`);
    assert.ok(!output.warnings.some(w => w.code === 'W001'), `Should not have W001: ${JSON.stringify(output.warnings)}`);
  });

  // ─── Check 3: HUNTMAP.md exists ───────────────────────────────────────────

  test('errors when HUNTMAP.md is missing', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalStateMd(tmpDir);
    writeValidConfigJson(tmpDir);
    // No HUNTMAP.md
  ...
  test('reports missing HUNTMAP.md in hunt mode with hunt-native fix guidance', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalStateMd(tmpDir);
    writeValidConfigJson(tmpDir);
    ...
    const output = JSON.parse(result.output);
    const e003 = output.errors.find(e => e.code === 'E003');
    assert.ok(e003, `Expected E003 in errors: ${JSON.stringify(output.errors)}`);
    assert.ok(e003.message.includes('HUNTMAP.md'));
    assert.ok(e003.fix.includes('/hunt:new-case'));
  });
...
`
  ...
  test('warns when legacy workflow.verifier key is present', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalHuntmap(tmpDir, ['1']);
    writeMinimalStateMd(tmpDir);
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ workflow: { verifier: false } }, null, 2)
    );
    ...
    const output = JSON.parse(result.output);
    assert.ok(output.warnings.some(w => w.code === 'W011'), `Expected W011 in warnings: ${JSON.stringify(output.warnings)}`);
  });

  test('warns when workflow.discuss_mode uses legacy standard value', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalHuntmap(tmpDir, ['1']);
    writeMinimalStateMd(tmpDir);
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ workflow: { discuss_mode: 'standard' } }, null, 2)
    );
    ...
    const output = JSON.parse(result.output);
    assert.ok(output.warnings.some(w => w.code === 'W012'), `Expected W012 in warnings: ${JSON.stringify(output.warnings)}`);
  });
  ...
  test('warns about phase in HUNTMAP but not on disk', () => {
    writeMinimalMissionMd(tmpDir);
    // HUNTMAP mentions Phase 5 but no 05-xxx dir
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
  ...
  test('warns about phase on disk but not in HUNTMAP', () => {
    writeMinimalMissionMd(tmpDir);
    // HUNTMAP has no phases
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'HUNTMAP.md'),
      '# Huntmap\n\nNo phases listed.\n'
    );
    writeMinimalStateMd(tmpDir, '# Session State\n\nNo phase refs.\n');
    writeValidConfigJson(tmpDir);
    // Orphan phase dir not in HUNTMAP
  ...
  test("returns 'healthy' when all checks pass", () => {
    ...
    writeMinimalStateMd(tmpDir, '# Session State\n\nPhase 1 in progress.\n');
    writeValidConfigJson(tmpDir);
    // Create valid phase dir matching HUNTMAP
...
describe('validate health --repair command', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    // Set up base project with HUNTMAP and MISSION.md so repairs are triggered
    // (E001, E003 are not repairable so we always need .planning/ and HUNTMAP.md)
    writeMinimalMissionMd(tmpDir);
    writeMinimalHuntmap(tmpDir, ['1']);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases', '01-a'), { recursive: true });
  });
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    const result = runThruntTools('validate health --repair', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.strictEqual(diskConfig.workflow.validator, true, 'workflow.validator should default to true');
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'thrunt/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'thrunt/{milestone}-{slug}');
  });
  ...
  test('normalizes legacy verifier and discuss_mode values during repair', () => {
    writeMinimalMissionMd(tmpDir);
    writeMinimalHuntmap(tmpDir, ['1']);
    writeMinimalStateMd(tmpDir);
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    fs.writeFileSync(
      configPath,
      JSON.stringify({ workflow: { verifier: false, discuss_mode: 'standard' } }, null, 2)
    );
    ...
    assert.ok(
      output.repairs_performed.some(a => a.action === 'normalizeConfigSchema' && a.success),
      `Expected normalizeConfigSchema repair: ${JSON.stringify(output.repairs_performed)}`
    );

    const diskConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
    assert.strictEqual(diskConfig.workflow.validator, false);
    assert.strictEqual(diskConfig.workflow.verifier, undefined);
    assert.strictEqual(diskConfig.workflow.discuss_mode, 'discuss');
  });
