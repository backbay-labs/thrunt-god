...
그래서 THRUNT를 만들었습니다. 복잡함은 시스템 안에 있습니다. 워크플로우에 있는 게 아니라. 뒤에서 컨텍스트 엔지니어링, XML 프롬프트 포맷팅, 서브에이전트 오케스트레이션, 상태 관리가 돌아갑니다. 겉에서 보이는 건 그냥 몇 가지 명령어뿐입니다.
...
THRUNT가 그걸 고칩니다. Claude Code를 신뢰할 수 있게 만드는 컨텍스트 엔지니어링 레이어입니다. 아이디어를 설명하면 시스템이 필요한 걸 다 뽑아내고, Claude Code가 일을 시작합니다.
...
> [!NOTE]
> Codex 설치는 커스텀 프롬프트 대신 스킬(`skills/thrunt-*/SKILL.md`)을 사용합니다.

### 업데이트 유지

THRUNT는 빠르게 발전합니다. 주기적으로 업데이트하세요:
...
```bash
# Claude Code
npx thrunt-god --claude --global   # ~/.claude/에 설치
npx thrunt-god --claude --local    # ./.claude/에 설치

# OpenCode (오픈소스, 무료 모델)
npx thrunt-god --opencode --global # ~/.config/opencode/에 설치

# Gemini CLI
npx thrunt-god --gemini --global   # ~/.gemini/에 설치

# Codex (스킬 우선)
npx thrunt-god --codex --global    # ~/.codex/에 설치
npx thrunt-god --codex --local     # ./.codex/에 설치

# Copilot (GitHub Copilot CLI)
npx thrunt-god --copilot --global  # ~/.github/에 설치
npx thrunt-god --copilot --local   # ./.github/에 설치

# Cursor CLI
npx thrunt-god --cursor --global      # ~/.cursor/에 설치
npx thrunt-god --cursor --local       # ./.cursor/에 설치

# Antigravity (Google, 스킬 우선, Gemini 기반)
npx thrunt-god --antigravity --global # ~/.gemini/antigravity/에 설치
npx thrunt-god --antigravity --local  # ./.agent/에 설치

# 전체 런타임
npx thrunt-god --all --global      # 모든 디렉터리에 설치
```
...
THRUNT는 마찰 없는 자동화를 위해 설계되었습니다. Claude Code를 다음과 같이 실행하세요:
...
> [!TIP]
> 이게 THRUNT를 사용하는 방법입니다 — `date`와 `git commit` 50번을 승인하러 멈추면 의미가 없습니다.
...
> **이미 코드가 있나요?** 먼저 `/hunt:map-environment`를 실행하세요. 병렬 에이전트를 생성해 스택, 아키텍처, 컨벤션, 고려사항을 분석합니다. 그러면 `/hunt:new-program`가 코드베이스를 파악한 상태에서 시작되고 — 질문은 추가하는 것에 집중되고, 기획 시 자동으로 기존 패턴을 불러옵니다.
...
**생성 파일:** `MISSION.md`, `HYPOTHESES.md`, `HUNTMAP.md`, `STATE.md`, `.planning/research/`
...
> **가정 모드:** 질문보다 코드베이스 분석을 선호하나요? `/thrunt:settings`에서 `workflow.discuss_mode`를 `assumptions`로 설정하세요. 시스템이 코드를 읽고 하려는 것과 이유를 제시한 다음 틀린 부분만 수정을 요청합니다. [논의 모드](docs/ko-KR/workflow-discuss-mode.md) 참조.
...
**생성 파일:** `{phase_num}-{N}-SUMMARY.md`, `{phase_num}-FINDINGS.md`
...
모든 게 통과하면 다음으로 넘어갑니다. 뭔가 깨졌으면 직접 디버그하지 않아도 됩니다 — 생성된 수정 계획으로 `/hunt:run`만 다시 실행하면 됩니다.

**생성 파일:** `{phase_num}-EVIDENCE_REVIEW.md`, 문제 발견 시 수정 계획
...
/hunt:publish 2                  # 검증된 작업으로 PR 생성
...
/thrunt:complete-milestone
/hunt:new-program
```

또는 THRUNT가 다음 단계를 자동으로 파악하게 합니다:

```
/thrunt:next                    # 다음 단계 자동 감지 및 실행
```

마일스톤이 완료될 때까지 **논의 → 기획 → 실행 → 검증 → 출시** 반복.

논의 중에 더 빠르게 진행하고 싶다면 `/hunt:shape-hypothesis <n> --batch`를 사용해 하나씩이 아닌 소그룹으로 한 번에 답할 수 있습니다.

각 단계는 사용자 입력(논의), 적절한 리서치(기획), 깔끔한 실행(실행), 사람의 검증(검증)을 거칩니다. 컨텍스트는 새롭게 유지됩니다. 품질도 높게 유지됩니다.

모든 단계가 끝나면 `/thrunt:complete-milestone`이 마일스톤을 아카이브하고 릴리스에 태그를 답니다.

그다음 `/hunt:new-program`으로 다음 버전을 시작합니다 — `new-program`와 같은 흐름이지만 기존 코드베이스를 위한 것입니다. 다음에 만들 것을 설명하면 시스템이 도메인을 리서치하고, 요구사항을 스코핑하고, 새 로드맵을 만듭니다. 각 마일스톤은 깔끔한 사이클입니다: 정의 → 구축 → 출시.
...
빠른 모드는 THRUNT 보장 (원자적 커밋, 상태 추적)을 더 빠른 경로로 제공합니다:
...
| 파일 | 역할 |
|------|--------------|
| `MISSION.md` | 프로젝트 비전, 항상 로드 |
| `research/` | 생태계 지식 (스택, 기능, 아키텍처, 주의사항) |
| `HYPOTHESES.md` | 단계 추적성이 있는 스코핑된 v1/v2 요구사항 |
| `HUNTMAP.md` | 방향과 완료된 것 |
...
| 명령어 | 역할 |
|---------|------------|
| `/hunt:new-program [--auto]` | 전체 초기화: 질문 → 리서치 → 요구사항 → 로드맵 |
| `/hunt:shape-hypothesis [N] [--auto] [--analyze]` | 기획 전 구현 결정 캡처 (`--analyze`는 트레이드오프 분석 추가) |
| `/hunt:plan [N] [--auto] [--reviews]` | 단계에 대한 리서치 + 기획 + 검증 (`--reviews`는 코드베이스 리뷰 결과 로드) |
| `/hunt:run <N>` | 병렬 웨이브로 모든 계획 실행, 완료 시 검증 |
| `/hunt:validate-findings [N]` | 수동 사용자 인수 테스트 ¹ |
| `/hunt:publish [N] [--draft]` | 자동 생성된 본문으로 검증된 단계 작업에서 PR 생성 |
| `/thrunt:next` | 다음 논리적 워크플로우 단계로 자동 진행 |
| `/thrunt:fast <text>` | 인라인 사소한 작업 — 기획 완전 건너뛰고 즉시 실행 |
| `/thrunt:audit-milestone` | 마일스톤이 완료 정의를 달성했는지 검증 |
| `/thrunt:complete-milestone` | 마일스톤 아카이브, 릴리스 태그 |
| `/hunt:new-program [name]` | 다음 버전 시작: 질문 → 리서치 → 요구사항 → 로드맵 |
| `/thrunt:forensics [desc]` | 실패한 워크플로우 실행의 사후 조사 (막힌 루프, 누락된 아티팩트, git 이상 진단) |
| `/thrunt:milestone-summary [version]` | 팀 온보딩 및 리뷰를 위한 종합 프로젝트 요약 생성 |

### 워크스트림

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:workstreams list` | 모든 워크스트림과 상태 표시 |
| `/thrunt:workstreams create <name>` | 병렬 마일스톤 작업을 위한 네임스페이스 워크스트림 생성 |
| `/thrunt:workstreams switch <name>` | 활성 워크스트림 전환 |
| `/thrunt:workstreams complete <name>` | 워크스트림 완료 및 병합 |

### 멀티 프로젝트 워크스페이스

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:new-workspace` | 저장소 복사본으로 격리된 워크스페이스 생성 (worktrees 또는 clones) |
| `/thrunt:list-workspaces` | 모든 THRUNT 워크스페이스와 상태 표시 |
| `/thrunt:remove-workspace` | 워크스페이스 제거 및 worktree 정리 |

### UI 디자인

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:ui-phase [N]` | 프론트엔드 단계를 위한 UI 디자인 계약 (UI-SPEC.md) 생성 |
| `/thrunt:ui-review [N]` | 구현된 프론트엔드 코드의 소급적 6가지 기준 시각 감사 |

### 탐색

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:progress` | 지금 어디에 있나? 다음은? |
| `/thrunt:next` | 상태 자동 감지 및 다음 단계 실행 |
| `/thrunt:help` | 모든 명령어와 사용 가이드 표시 |
| `/thrunt:update` | 변경 로그 미리보기와 함께 THRUNT 업데이트 |
| `/thrunt:join-discord` | THRUNT Discord 커뮤니티 참여 |
| `/thrunt:manager` | 여러 단계 관리를 위한 대화형 커맨드 센터 |

### 브라운필드

| 명령어 | 역할 |
|---------|------------|
| `/hunt:map-environment [area]` | new-program 전 기존 코드베이스 분석 |

### 단계 관리

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:add-phase` | 로드맵에 단계 추가 |
| `/thrunt:insert-phase [N]` | 단계 사이에 긴급 작업 삽입 |
| `/thrunt:remove-phase [N]` | 미래 단계 제거, 번호 재정렬 |
| `/thrunt:list-phase-assumptions [N]` | 기획 전 Claude의 의도된 접근 방식 확인 |
| `/thrunt:plan-milestone-gaps` | 감사에서 발견된 갭을 해소하기 위한 단계 생성 |

### 세션

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:pause-work` | 단계 중간에 멈출 때 핸드오프 생성 (HANDOFF.json 작성) |
| `/thrunt:resume-work` | 마지막 세션에서 복원 |
| `/thrunt:session-report` | 수행한 작업과 결과가 담긴 세션 요약 생성 |

### 코드 품질

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:review` | 현재 단계 또는 브랜치의 Cross-AI 피어 리뷰 |
| `/thrunt:pr-branch` | `.planning/` 커밋을 필터링한 깔끔한 PR 브랜치 생성 |
| `/thrunt:audit-evidence` | 검증 부채 감사 — Evidence Review가 누락된 단계 찾기 |

### 백로그 및 스레드

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:plant-seed <idea>` | 트리거 조건이 있는 아이디어 저장 — 때가 되면 알아서 올라옴 |
| `/thrunt:add-backlog <desc>` | 백로그 파킹 롯에 아이디어 추가 (999.x 번호 지정, 활성 시퀀스 외부) |
| `/thrunt:review-backlog` | 백로그 항목 리뷰 및 활성 마일스톤으로 승격하거나 오래된 항목 제거 |
| `/thrunt:thread [name]` | 지속적 컨텍스트 스레드 — 여러 세션에 걸친 작업을 위한 가벼운 크로스 세션 지식 |

### 유틸리티

| 명령어 | 역할 |
|---------|------------|
| `/thrunt:settings` | 모델 프로필 및 워크플로우 에이전트 설정 |
| `/thrunt:set-profile <profile>` | 모델 프로필 전환 (quality/balanced/budget/inherit) |
| `/thrunt:add-todo [desc]` | 나중을 위한 아이디어 캡처 |
| `/thrunt:check-todos` | 대기 중인 할 일 목록 |
| `/thrunt:debug [desc]` | 지속적 상태를 이용한 체계적 디버깅 |
| `/thrunt:do <text>` | 자유 형식 텍스트를 적절한 THRUNT 명령어로 자동 라우팅 |
| `/thrunt:note <text>` | 마찰 없는 아이디어 캡처 — 추가, 목록, 또는 할 일로 승격 |
| `/thrunt:quick [--full] [--discuss] [--research]` | THRUNT 보장과 함께 임시 작업 실행 (`--full`은 계획 확인 및 검증 추가, `--discuss`는 먼저 컨텍스트 수집, `--research`는 기획 전 접근법 조사) |
| `/thrunt:health [--repair]` | `.planning/` 디렉터리 무결성 검증, `--repair`로 자동 복구 |
| `/thrunt:stats` | 프로젝트 통계 표시 — 단계, 계획, 요구사항, git 지표 |
| `/thrunt:profile-user [--questionnaire] [--refresh]` | 개인화된 응답을 위해 세션 분석에서 개발자 행동 프로필 생성 |
...
THRUNT는 프로젝트 설정을 `.planning/config.json`에 저장합니다. `/hunt:new-program` 중에 설정하거나 나중에 `/thrunt:settings`로 업데이트할 수 있습니다. 전체 config 스키마, 워크플로우 토글, git 브랜칭 옵션, 에이전트별 모델 분석은 [사용자 가이드](docs/ko-KR/USER-GUIDE.md#configuration-reference)를 참조하세요.
...
| `workflow.validator` | `true` | 실행 후 필수 사항이 전달됐는지 확인 |
...
| `workflow.skip_discuss` | `false` | 자율 모드에서 shape-hypothesis 건너뛰기 |
| `workflow.text_mode` | `false` | 원격 세션을 위한 텍스트 전용 모드 (TUI 메뉴 없음) |

`/thrunt:settings`로 토글하거나 호출별로 재정의하세요:
...
실행 중 THRUNT의 브랜치 처리 방식을 제어합니다.

| 설정 | 옵션 | 기본값 | 역할 |
|---------|---------|---------|--------------|
| `git.branching_strategy` | `none`, `phase`, `milestone` | `none` | 브랜치 생성 전략 |
| `git.phase_branch_template` | string | `thrunt/phase-{phase}-{slug}` | 단계 브랜치 템플릿 |
| `git.milestone_branch_template` | string | `thrunt/{milestone}-{slug}` | 마일스톤 브랜치 템플릿 |

**전략:**
- **`none`** — 현재 브랜치에 커밋 (기본 THRUNT 동작)
- **`phase`** — 단계당 브랜치 생성, 단계 완료 시 병합
- **`milestone`** — 전체 마일스톤을 위한 하나의 브랜치 생성, 완료 시 병합

마일스톤 완료 시 THRUNT가 스쿼시 병합 (권장) 또는 이력과 함께 병합을 제안합니다.
...
THRUNT는 v1.27부터 심층 방어 보안을 포함합니다:
...
- **PreToolUse 프롬프트 가드 훅** — `thrunt-prompt-guard`가 `.planning/`에 대한 쓰기에서 내장된 인젝션 벡터 스캔 (권고적, 차단하지 않음)
...
> [!NOTE]
> THRUNT는 LLM 시스템 프롬프트가 되는 마크다운 파일을 생성하기 때문에, 기획 아티팩트에 들어가는 사용자 제어 텍스트는 잠재적인 간접 프롬프트 인젝션 벡터가 됩니다. 이 보호 장치들은 여러 레이어에서 그런 벡터를 잡도록 설계되었습니다.

### 민감한 파일 보호

THRUNT의 코드베이스 매핑 및 분석 명령어는 프로젝트를 이해하기 위해 파일을 읽습니다. **비밀이 담긴 파일**을 Claude Code의 거부 목록에 추가해 보호하세요:
...
> [!IMPORTANT]
> THRUNT에는 비밀 커밋에 대한 내장 보호 장치가 있지만, 심층 방어가 모범 사례입니다. 민감한 파일에 대한 읽기 접근을 거부하는 것을 첫 번째 방어선으로 삼으세요.
...
- `~/.claude/commands/thrunt/` (전역) 또는 `./.claude/commands/thrunt/` (로컬)에 파일이 있는지 확인하세요
- Codex의 경우 `~/.codex/skills/thrunt-*/SKILL.md` (전역) 또는 `./.codex/skills/thrunt-*/SKILL.md` (로컬)에 스킬이 있는지 확인하세요

**명령어가 예상대로 작동하지 않나요?**
- `/thrunt:help`를 실행해 설치 확인
- `npx thrunt-god`를 다시 실행해 재설치
...
다른 설정은 그대로 유지하면서 THRUNT의 모든 명령어, 에이전트, 훅, 설정을 제거합니다.

---

## 커뮤니티 포트

OpenCode, Gemini CLI, Codex는 이제 `npx thrunt-god`를 통해 기본 지원됩니다.

이 커뮤니티 포트들이 멀티 런타임 지원의 선구자였습니다:

| 프로젝트 | 플랫폼 | 설명 |
|---------|----------|-------------|
| [thrunt-opencode](https://github.com/rokicool/thrunt-opencode) | OpenCode | 최초 OpenCode 적응 |
| thrunt-gemini (아카이브됨) | Gemini CLI | uberfuzzy의 최초 Gemini 적응 |
...
**Claude Code는 강력합니다. THRUNT가 그걸 신뢰할 수 있게 만듭니다.**
